...
void HandleRenderFaceAdjustments( FACETYPE *pFace, BOOLEAN fDisplayBuffer, BOOLEAN fUseExternBuffer, UINT32 uiBuffer, INT16 sFaceX, INT16 sFaceY, UINT16 usEyesX, UINT16 usEyesY, UINT32 uiFaceShade)
{
	...
	if ( pFace->ubSoldierID != NOBODY )
	{
		...
		if (pSoldier->vitals().health() > 0 && !(pFace->uiFlags & FACE_BIGFACE))
		{
			...
			if (UsingFoodSystem())
			{
				if (pSoldier->condition().drinkLevel() < FoodMoraleMods[FOOD_MERC_START_SHOW_HUNGER_SYMBOL].bThreshold)
				{
					DoRightIcon(uiRenderBuffer, pFace, sFaceX, sFaceY, bNumRightIcons, 9);
					bNumRightIcons++;
				}

				if (pSoldier->condition().foodLevel() < FoodMoraleMods[FOOD_MERC_START_SHOW_HUNGER_SYMBOL].bThreshold)
				{
