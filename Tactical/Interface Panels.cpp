...
static SOLDIERTYPE* EnsureSMCurrentMerc()
{
	...
	// a new soldier occupying the same legacy slot.
...
void CheckForDisabledForGiveItem( )
{
	...
	{
		gfSMDisableForItems = !CanMercInteractWithSelectedShopkeeper(currentMerc);
		return;
	}
...
void SetSMPanelCurrentMerc( SoldierID ubNewID )
{
	...
	HandleCompatibleAmmoUI(GetSMCurrentMerc(), NO_SLOT, FALSE);
	if (gpItemPointer != NULL)
	{
...
void EnableSMPanelButtons( BOOLEAN fEnable , BOOLEAN fFromItemPickup )
{
	...
	if ( gsCurInterfacePanel == SM_PANEL )
	{
		if ( fFromItemPickup )
		{
			...
			if ( gpItemPointer != NULL )
			{
				ReevaluateItemHatches( GetSMCurrentMerc(), fEnable );

				// Turn it all false first....
				InternalHandleCompatibleAmmoUI( GetSMCurrentMerc(), gpItemPointer, FALSE );
				InternalHandleCompatibleAmmoUI( GetSMCurrentMerc(), gpItemPointer, TRUE );
...
//	InitializeSMPanelCoords( );
  ...
  fDisplayOverheadMap = TRUE;
	...
	if ( IsMouseInRegion( &gSM_SELMERCPanelRegion ) )
	{
		HandleMouseOverSoldierFaceForContMove( GetSMCurrentMerc(), TRUE );
	}
...
BOOLEAN ShutdownSMPanel( )
{
	...
	HandleMouseOverSoldierFaceForContMove( GetSMCurrentMerc(), FALSE );
...
void RenderSMPanel( BOOLEAN *pfDirty )
{
	...
	if ( gfCheckForMouseOverItem )
	{
		...
		{
			if ( HandleCompatibleAmmoUI( GetSMCurrentMerc(), (INT8)gbCheckForMouseOverItemPos, TRUE ) )
			{
				(*pfDirty) = DIRTYLEVEL2;
			}
	...
	HandleNewlyAddedItems( GetSMCurrentMerc(), pfDirty );
	...
	if ( *pfDirty == DIRTYLEVEL2 )
	{
		...
		if ( InItemDescriptionBox( ) )
		{
			...
			RenderSoldierFace( GetSMCurrentMerc(), SM_SELMERC_FACE_X, SM_SELMERC_FACE_Y, TRUE );
		...
		else
		{
			...
			RenderInvBodyPanel( GetSMCurrentMerc(), SM_BODYINV_X, SM_BODYINV_Y );

			// CHECK FOR PANEL STUFF / DEATHS / CLOSURES
			CheckForFacePanelStartAnims( GetSMCurrentMerc(), SM_SELMERC_FACE_X, SM_SELMERC_FACE_Y );
			...
			if( g_lang == i18n::Lang::zh ) {
				...
				UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Agility), (BOOLEAN)(GetSMCurrentMerc()->usValueGoneUp & AGIL_INCREASE ? TRUE : FALSE), (BOOLEAN)((gGameOptions.fNewTraitSystem && (GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_AGILITY] > 0)) ? TRUE : FALSE), GetSMCurrentMerc()->condition().extraAgility() != 0 ); // SANDRO
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Dexterity), (BOOLEAN)(GetSMCurrentMerc()->usValueGoneUp & DEX_INCREASE ? TRUE : FALSE), (BOOLEAN)((gGameOptions.fNewTraitSystem && (GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_DEXTERITY] > 0)) ? TRUE : FALSE), GetSMCurrentMerc()->condition().extraDexterity() != 0 ); // SANDRO
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Leadership), ( BOOLEAN )( GetSMCurrentMerc()->usValueGoneUp & LDR_INCREASE? TRUE: FALSE ), ( BOOLEAN ) ( ( gGameOptions.fNewTraitSystem && ( GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_LEADERSHIP] > 0 )) ? TRUE : FALSE), FALSE); // SANDRO
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Wisdom), (BOOLEAN)(GetSMCurrentMerc()->usValueGoneUp & WIS_INCREASE ? TRUE : FALSE), (BOOLEAN)((gGameOptions.fNewTraitSystem && (GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_WISDOM] > 0)) ? TRUE : FALSE), GetSMCurrentMerc()->condition().extraWisdom() != 0 ); // SANDRO
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Level), ( BOOLEAN ) ( GetSMCurrentMerc()->usValueGoneUp & LVL_INCREASE? TRUE: FALSE ),  FALSE , FALSE);
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Marksmanship), ( BOOLEAN ) ( GetSMCurrentMerc()->usValueGoneUp & MRK_INCREASE? TRUE: FALSE ), ( BOOLEAN ) ( ( gGameOptions.fNewTraitSystem && ( GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_MARKSMANSHIP] > 0 )) ? TRUE : FALSE), FALSE); // SANDRO
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Mechanical), ( BOOLEAN ) ( GetSMCurrentMerc()->usValueGoneUp & MECH_INCREASE ? TRUE: FALSE ), ( BOOLEAN ) ( ( gGameOptions.fNewTraitSystem && ( GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_MECHANICAL] > 0 )) ? TRUE : FALSE), FALSE); // SANDRO
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Explosives), ( BOOLEAN ) ( GetSMCurrentMerc()->usValueGoneUp & EXP_INCREASE? TRUE: FALSE ), ( BOOLEAN ) ( ( gGameOptions.fNewTraitSystem && ( GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_EXPLOSIVES] > 0 )) ? TRUE : FALSE), FALSE); // SANDRO
			...
			UpdateStatColor( GetSMCurrentMerc()->statProgress().changedAt(SoldierStatProgressComponent::Stat::Medical), ( BOOLEAN ) ( GetSMCurrentMerc()->usValueGoneUp & MED_INCREASE? TRUE: FALSE ), ( BOOLEAN ) ( ( gGameOptions.fNewTraitSystem && ( GetSMCurrentMerc()->vitals().criticalStatDamage()[DAMAGED_STAT_MEDICAL] > 0 )) ? TRUE : FALSE), FALSE); // SANDRO
			...
			swprintf( sString, L"%3d", ArmourPercent( GetSMCurrentMerc() ) );
			...
			swprintf( sString, L"%3d", CalculateCarriedWeight( GetSMCurrentMerc() ) );
	...
	if ( *pfDirty != DIRTYLEVEL0 )
	{
		...
		else if ( gGameOptions.fNewTraitSystem && HAS_SKILL_TRAIT( GetSMCurrentMerc(), STEALTHY_NT ))
		{
			bonusstealth += gSkillTraitValues.ubSTBonusToMoveQuietly;
		}
		else if ( !gGameOptions.fNewTraitSystem && HAS_SKILL_TRAIT( GetSMCurrentMerc(), STEALTHY_OT ))
		{
			bonusstealth += 25 * NUM_SKILL_TRAITS( GetSMCurrentMerc(), STEALTHY_OT );
		}
		...
		FLOAT totalweight = (FLOAT)GetTotalWeight( GetSMCurrentMerc() );
		...
		else
		{
...

			{
				...
				GetSMCurrentMerc()->PrintDiseaseDesc( atStr );
		...
		if ( !( GetSMCurrentMerc()->flags.uiStatusFlags & SOLDIER_DEAD ) )
		{
			if ( IsJa2TacticalTurnBasedCombat() && GetSMCurrentMerc()->vitals().health() >= OKLIFE )
			{
				...
				{
					if ( MercDruggedOrDrunk( GetSMCurrentMerc() ) )
					{
						SetFontBackground( FONT_MCOLOR_BLACK );
				...
				gprintfRestore( SM_SELMERC_AP_X, SM_SELMERC_AP_Y, L"%3d", GetUIApsToDisplay( GetSMCurrentMerc() ) );
				VarFindFontCenterCoordinates( SM_SELMERC_AP_X, SM_SELMERC_AP_Y, SM_SELMERC_AP_WIDTH, SM_SELMERC_AP_HEIGHT, TINYFONT1, &sFontX, &sFontY, L"%d", GetUIApsToDisplay( GetSMCurrentMerc() ) );
				mprintf( sFontX, SM_SELMERC_AP_Y, L"%d", GetUIApsToDisplay( GetSMCurrentMerc() ) );
			}
 ...
 */
	...
	//HandlePanelFaceAnimations( GetSMCurrentMerc() );

	HandleSoldierFaceFlash( GetSMCurrentMerc(), SM_SELMERC_FACE_X, SM_SELMERC_FACE_Y );

	// Render items in guy's hand!
	HandleRenderInvSlots( GetSMCurrentMerc(), *pfDirty );
...
void SMInvMoveCallback( MOUSE_REGION * pRegion, INT32 iReason )
{
	...
	if (iReason == MSYS_CALLBACK_REASON_LOST_MOUSE )
	{
		...
		if ( gpItemPointer == NULL )
		{
			HandleCompatibleAmmoUI( GetSMCurrentMerc(), (INT8)uiHandPos, FALSE );
			gfCheckForMouseOverItem = FALSE;
			fInterfacePanelDirty = DIRTYLEVEL2;
...
void SMInvMoveCammoCallback( MOUSE_REGION * pRegion, INT32 iReason )
{
	...
	if (iReason == MSYS_CALLBACK_REASON_LOST_MOUSE )
	{
		...
		{
			//gfSM_HandInvDispText[ uiHandPos ] = 1;
			HandleCompatibleAmmoUI(GetSMCurrentMerc(), (INT8)NO_SLOT, FALSE);
			gfCheckForMouseOverItem = FALSE;
		}
	}
}
...
{
	// Try to place here
	if ( PlaceObject( GetSMCurrentMerc(), ubHandPos, gpItemPointer ) )
	{
		if ( fDeductPoints )
		{
			// Deduct points
		...
		HandleTacticalEffectsOfEquipmentChange( GetSMCurrentMerc(), ubHandPos, usOldItemIndex, usNewItemIndex );
...
void SMInvClickCallback( MOUSE_REGION * pRegion, INT32 iReason )
{
	...
	if (iReason & MSYS_CALLBACK_REASON_LBUTTON_DWN )
	{
		...
		if ( gpItemPointer == NULL )
		{
			...
			if ( HandleNailsVestFetish( GetSMCurrentMerc(), uiHandPos, NOTHING ) )
			{
				return;
			}
			...
			BeginItemPointer( GetSMCurrentMerc(), (UINT8)uiHandPos );
			...
			HandleTacticalEffectsOfEquipmentChange( GetSMCurrentMerc(), uiHandPos, usOldItemIndex, NOTHING );
		...
		else	// item in cursor
		{
			...
			if ( ubSrcID == ubDestID )
			{
				if(!CanItemFitInPosition(GetSMCurrentMerc(), gpItemPointer, (INT8)uiHandPos, FALSE))//dnl ch66 070913
					return;
				if (INV_AP_COST)
...
//				fOKToGo = TRUE;
			...
			if ( fOKToGo )
			{
				...
				// OK, check if this is Nails, and we're in the vest position , don't allow it to come off....
				if ( HandleNailsVestFetish( GetSMCurrentMerc(), uiHandPos, usNewItemIndex ) )
				{
					return;
				}
				...
				if ( UIHandleItemPlacement( (UINT8) uiHandPos, usOldItemIndex, usNewItemIndex, fDeductPoints ) )
				{
					...
					if( guiTacticalInterfaceFlags & INTERFACE_SHOPKEEP_INTERFACE )
					{
						SetNewItem( GetSMCurrentMerc(), ( UINT8 ) uiHandPos, fNewItem );
				...
				/*
				// Try to place here
				if ( PlaceObject( GetSMCurrentMerc(), (UINT8)uiHandPos, gpItemPointer ) )
				{
					...
					HandleTacticalEffectsOfEquipmentChange( GetSMCurrentMerc(), uiHandPos, usOldItemIndex, usNewItemIndex );
		...
		*/
		...
		else
		{
			if ( !InItemStackPopup( )	)
			{
				...
				//InitItemStackPopup( GetSMCurrentMerc(), (UINT8)uiHandPos, SM_ITEMDESC_START_X, SM_ITEMDESC_START_Y, SM_ITEMDESC_WIDTH, SM_ITEMDESC_HEIGHT );
				...
				InitItemStackPopup( GetSMCurrentMerc(), (UINT8)uiHandPos, inv_interface_start_x + xResOffset, INV_INTERFACE_START_Y, invWidth, ( SCREEN_HEIGHT - INV_INTERFACE_START_Y ) );
...
void HandleMouseOverSoldierFaceForContMove( SOLDIERTYPE *pSoldier, BOOLEAN fOn )
{
	...
	if ( fOn )
	{
		...
		if ( CheckForMercContMove( pSoldier ) )
		{
			...
			PlotPath( pSoldier, sGridNo, NO_COPYROUTE, PLOT, TEMPORARY, (UINT16)pSoldier->movement().mode(), NOT_STEALTH, FORWARD, pSoldier->actionPoints().current() );
		}
	}
	else
	{
...
void BtnPrevMercCallback(GUI_BUTTON *btn,INT32 reason)
{
	...
	else if(reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		SoldierID sID = FindPrevActiveAndAliveMerc( GetSMCurrentMerc(), TRUE, TRUE );
...
void BtnNextMercCallback(GUI_BUTTON *btn,INT32 reason)
{
	...
	else if(reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		SoldierID sID = FindNextActiveAndAliveMerc( GetSMCurrentMerc(), TRUE, TRUE );
...
void HandleMouseOverTeamFaceForContMove( BOOLEAN fOn )
{
	...
	if ( fOn )
	{
		...
		if ( CheckForMercContMove( GetSMCurrentMerc() ) )
		{
			...
			// While our mouse is here, draw a path!
			PlotPath( GetSMCurrentMerc(), GetSMCurrentMerc()->pathing().finalDestinationGrid(), NO_COPYROUTE, PLOT, TEMPORARY, (UINT16)GetSMCurrentMerc()->movement().mode(), NOT_STEALTH, FORWARD, GetSMCurrentMerc()->actionPoints().current() );
		}
	}
	else
	{
...
//			InvSlot.ubItemQuality = GetSMCurrentMerc()->inv[ uiHandPos ].gun.bGunStatus;
		...
		if ( gpItemPointer == NULL )
		{
			...
			BeginKeyRingItemPointer( GetItemPopupSoldier(), (UINT8)usOldItemIndex );
			//BeginItemPointer( GetSMCurrentMerc(), (UINT8)uiHandPos );
		...
		else
		{
			...
			if ( fOKToGo )
			{
				...
				{
					// Swap!
					SwapKeysToSlot( GetItemPopupSoldier(), (INT8) uiKeyRing, gpItemPointer );
	...
	else if (iReason & MSYS_CALLBACK_REASON_RBUTTON_UP && fRightDown )
	{
		...
		if ( !InItemDescriptionBox( ) )
		{
			if ( guiTacticalInterfaceFlags & INTERFACE_MAPSCREEN )
			{
				//InitKeyItemDescriptionBox( GetItemPopupSoldier(), (UINT8)uiKeyRing, MAP_ITEMDESC_START_X, MAP_ITEMDESC_START_Y, 0 );
		}
		else
		{
				InitKeyItemDescriptionBox( GetItemPopupSoldier(), (UINT8)uiKeyRing, SM_ITEMDESC_START_X, SM_ITEMDESC_START_Y, 0 );
		}
		}
	}
	else if (iReason & MSYS_CALLBACK_REASON_LOST_MOUSE )
...
//			else
			...
			InternalInitItemDescriptionBox( &gItemPointer, ITEMDESC_START_X, ITEMDESC_START_Y, 0, GetSMCurrentMerc() );
		}
	}
}
...
BOOLEAN HandleKlerykPistolet( SOLDIERTYPE *pSoldier, UINT32 uiHandPos, UINT16 usReplaceItem )
{
	...
	if ( pSoldier->ubProfile == 57 )
	{
		if ( uiHandPos == HANDPOS || uiHandPos == SECONDHANDPOS )
		{
			...
			{	//legion
				// Do we have nothing or the leather vest or kevlar leather vest?
				if ( usReplaceItem == 4520 ) //zloty pistolet items.xml
				{
					// This is good....
					fRefuse = FALSE;
				}
