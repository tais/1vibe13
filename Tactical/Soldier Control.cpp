...
// Conversion operator
TacticalActor& TacticalActor::operator=(const OLDSOLDIERTYPE_101& src)
{
	if ( (void*)this != (void*)&src ) {
		...
		memcpy( awareness().opponentKnowledge(), src.bOppList,
		        sizeof(INT8) * MAX_NUM_SOLDIERS );
		TranslateArrayFields( aiPlanning().patrolGrid(), src.usPatrolGrid,
		                      OLD_MAXPATROLGRIDS, INT16_INT32 );
		...
		animationActivity().paused() = src.fPauseAllAnimation;
		animationActivity().holdAttackerUntilDone() = src.fHoldAttackerUntilDone;
		animationActivity().turningToShoot() = src.fTurningToShoot;
		animationActivity().turningToFall() = src.fTurningToFall;
		animationActivity().turningUntilDone() = src.fTurningUntilDone;
		animationActivity().hitPhase() = src.fGettingHit;
		animationActivity().nonInterruptible() = src.fInNonintAnim;
		animationActivity().turningCostWaived() = src.fDontChargeTurningAPs;
		animationActivity().suppressionStanceChange() = src.fChangingStanceDueToSuppression;
		animationActivity().stanceCostWaived() = src.fDontChargeAPsForStanceChange;
		animationActivity().realtimeNonInterruptible() = src.fRTInNonintAnim;
		animationActivity().tryingToFall() = src.fTryingToFall;
		animationActivity().fallClockwise() = src.fFallClockwise;
		...
		this->roster().team() = src.bTeam;				// Team identifier
		...
		this->roster().soldierClass() = src.ubSoldierClass;									//admin, elite, troop (creature types?)
		...
		this->identity().incarnation() = src.uiUniqueSoldierIdValue; // the unique value every instance of a soldier gets - 1 is the first valid value
...
UINT32 TacticalActor::GetChecksum( )
{
	UINT32	uiChecksum = 1;
	UINT32	uiLoop;
	...
	uiChecksum += (this->identity().profile() + 1);

	UINT32 invsize = this->inventory().size( );
	for ( uiLoop = 0; uiLoop < invsize; ++uiLoop )
	{
		uiChecksum += this->inventory()[uiLoop].usItem;
		uiChecksum += this->inventory()[uiLoop].ubNumberOfObjects;
	}
...
void OLDSOLDIERTYPE_101::CopyOldInventoryToNew( )
{
	...
	inv.newItemCount(HELMETPOS) = DO_NOT_USE_bNewItemCount[OldInventory::HELMETPOS];
	inv.newItemCount(VESTPOS) = DO_NOT_USE_bNewItemCount[OldInventory::VESTPOS];
	inv.newItemCount(LEGPOS) = DO_NOT_USE_bNewItemCount[OldInventory::LEGPOS];
	inv.newItemCount(HEAD1POS) = DO_NOT_USE_bNewItemCount[OldInventory::HEAD1POS];
	inv.newItemCount(HEAD2POS) = DO_NOT_USE_bNewItemCount[OldInventory::HEAD2POS];
	inv.newItemCount(HANDPOS) = DO_NOT_USE_bNewItemCount[OldInventory::HANDPOS];
	inv.newItemCount(SECONDHANDPOS) = DO_NOT_USE_bNewItemCount[OldInventory::SECONDHANDPOS];
	inv.newItemCount(BIGPOCK1POS) = DO_NOT_USE_bNewItemCount[OldInventory::BIGPOCK1POS];
	inv.newItemCount(BIGPOCK2POS) = DO_NOT_USE_bNewItemCount[OldInventory::BIGPOCK2POS];
	inv.newItemCount(BIGPOCK3POS) = DO_NOT_USE_bNewItemCount[OldInventory::BIGPOCK3POS];
	inv.newItemCount(BIGPOCK4POS) = DO_NOT_USE_bNewItemCount[OldInventory::BIGPOCK4POS];
	inv.newItemCount(SMALLPOCK1POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK1POS];
	inv.newItemCount(SMALLPOCK2POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK2POS];
	inv.newItemCount(SMALLPOCK3POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK3POS];
	inv.newItemCount(SMALLPOCK4POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK4POS];
	inv.newItemCount(SMALLPOCK5POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK5POS];
	inv.newItemCount(SMALLPOCK6POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK6POS];
	inv.newItemCount(SMALLPOCK7POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK7POS];
	inv.newItemCount(SMALLPOCK8POS) = DO_NOT_USE_bNewItemCount[OldInventory::SMALLPOCK8POS];

	inv.newItemCycleCount(HELMETPOS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::HELMETPOS];
	inv.newItemCycleCount(VESTPOS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::VESTPOS];
	inv.newItemCycleCount(LEGPOS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::LEGPOS];
	inv.newItemCycleCount(HEAD1POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::HEAD1POS];
	inv.newItemCycleCount(HEAD2POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::HEAD2POS];
	inv.newItemCycleCount(HANDPOS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::HANDPOS];
	inv.newItemCycleCount(SECONDHANDPOS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SECONDHANDPOS];
	inv.newItemCycleCount(BIGPOCK1POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::BIGPOCK1POS];
	inv.newItemCycleCount(BIGPOCK2POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::BIGPOCK2POS];
	inv.newItemCycleCount(BIGPOCK3POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::BIGPOCK3POS];
	inv.newItemCycleCount(BIGPOCK4POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::BIGPOCK4POS];
	inv.newItemCycleCount(SMALLPOCK1POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK1POS];
	inv.newItemCycleCount(SMALLPOCK2POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK2POS];
	inv.newItemCycleCount(SMALLPOCK3POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK3POS];
	inv.newItemCycleCount(SMALLPOCK4POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK4POS];
	inv.newItemCycleCount(SMALLPOCK5POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK5POS];
	inv.newItemCycleCount(SMALLPOCK6POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK6POS];
	inv.newItemCycleCount(SMALLPOCK7POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK7POS];
	inv.newItemCycleCount(SMALLPOCK8POS) = DO_NOT_USE_bNewItemCycleCount[OldInventory::SMALLPOCK8POS];
}
...
INT16 TacticalActor::CalcActionPoints( void )
{
	...
	if ( soldierVitals.breath() < 100 && !(this->status().flags() & SOLDIER_VEHICLE) && !AM_A_ROBOT(this))
	...
	ubMaxAPs = gubMaxActionPoints[ this->identity().bodyType() ];
	...
	else if ( this->status().flags() & SOLDIER_MONSTER )
	{
		...
		if ( this->identity().bodyType() == YAF_MONSTER || this->identity().bodyType() == YAM_MONSTER || this->identity().bodyType() == INFANT_MONSTER )
		{
			ubPoints = (ubPoints * APBPConstants[AP_YOUNG_MONST_FACTOR]) / 10;
		}

		// if frenzied, female monsters get more APs! (for young females, cumulative!)
		if ( this->morale().frenzied() )
		{
	...
	// SANDRO - squadleader's bonus to APs
			  ...
			  (this->roster().team() == ENEMY_TEAM || this->roster().team() == MILITIA_TEAM || this->roster().team() == gbPlayerNum) )
	{
	...
	else if ( this->roster().team() == MILITIA_TEAM )
	{
		// This is only affected from 2/5
		if ( this->roster().soldierClass() == SOLDIER_CLASS_GREEN_MILITIA && gGameExternalOptions.bGreenMilitiaAPsBonus != 0 )
			ubPoints += gGameExternalOptions.bGreenMilitiaAPsBonus;
		else if ( this->roster().soldierClass() == SOLDIER_CLASS_REG_MILITIA && gGameExternalOptions.bRegularMilitiaAPsBonus != 0 )
			ubPoints += gGameExternalOptions.bRegularMilitiaAPsBonus;
		else if ( this->roster().soldierClass() == SOLDIER_CLASS_ELITE_MILITIA && gGameExternalOptions.bVeteranMilitiaAPsBonus != 0 )
			ubPoints += gGameExternalOptions.bVeteranMilitiaAPsBonus;
	}
	//Kaiden: Players just max out normally unless drugged
	else if ( this->roster().team() == gbPlayerNum )
	{
...
void TacticalActor::CalcNewActionPoints( void )
{
	if ( gTacticalStatus.bBoxingState == BOXING || gTacticalStatus.bBoxingState == PRE_BOXING )
	{
	     ...
	     !this->drugState().magnitude(DRUG_EFFECT_AGI) )
	{
		...
		UINT16 usMaxActionPnts = gubMaxActionPoints[this->identity().bodyType()];
	...
	if ( this->featureFlags().primaryFlags() & SOLDIER_NO_AP )
	{
		this->featureFlags().primaryFlags() &= ~SOLDIER_NO_AP;
				...
				if ( pSoldier->awareness().opponentCount() > 0 )
				{
					...
					if ( closestEnemy != nullptr )
					{
						...
						if ( gGameExternalOptions.fNoEnemyAutoReadyWeapon == 0 )
						{
							...
							// Ready weapon
							fReturnVal = pSoldier->SoldierReadyWeapon( sTargetXPos, sTargetYPos, FALSE, AIDecideHipOrShoulderStance( pSoldier, closestEnemy->position().gridNo() ) );
...
void TacticalActor::EVENT_FireSoldierWeapon( INT32 sTargetGridNo )
{
	INT16 sTargetXPos, sTargetYPos;
	...
	//switch ( this->inventory()[ this->attackSelection().hand() ][0]->data.gun.ubGunAmmoType )
	//{
	...
	if ( this->attackSelection().weaponMode() == WM_ATTACHED_GL || this->attackSelection().weaponMode() == WM_ATTACHED_GL_BURST || this->attackSelection().weaponMode() == WM_ATTACHED_GL_AUTO )
		usItem = GetAttachedGrenadeLauncher( &this->inventory()[HANDPOS] );
	else
		usItem = this->inventory()[HANDPOS].usItem;
	...
	// IF WE ARE AN NPC, SLIDE VIEW TO SHOW WHO IS SHOOTING
	{
		...
		// else
		{
			if ( this->status().flags() & SOLDIER_MONSTER )
			{
				// Force our direction!
			...
			else
			{
				...
				else
				{
					...
					this->animationActivity().turningToShoot() = TRUE;
...
BOOLEAN TacticalActor::SoldierReadyWeapon( void )
{
	return(this->InternalSoldierReadyWeapon( (INT8)this->position().direction(), FALSE, FALSE ));
}

BOOLEAN TacticalActor::SoldierReadyWeapon( INT16 sTargetXPos, INT16 sTargetYPos, BOOLEAN fEndReady, BOOLEAN fRaiseToHipOnly )
{
	INT16								sFacingDir;
...
BOOLEAN TacticalActor::InternalSoldierReadyWeapon( UINT8 sFacingDir, BOOLEAN fEndReady, BOOLEAN fRaiseToHipOnly )
{
	UINT16 usAnimState;
	BOOLEAN	fReturnVal = FALSE;
	...
	if ( !fEndReady && this->position().direction() != sFacingDir )//dnl ch71 170913
	{
		...
		case ANIM_STAND:
			...
			if ( this->animationActivity().turningCostWaived() && !this->animationActivity().readyCostWaived() && usAnimState == INVALID_ANIMATION )
	...
	if (fReturnVal &&
		...
		this->inventory()[HANDPOS].usItem &&
		Item[this->inventory()[HANDPOS].usItem].usItemClass & IC_GUN)
	{
		CHAR8	zFilename[512];
		sprintf(zFilename, "");

		switch (Weapon[Item[this->inventory()[HANDPOS].usItem].ubClassIndex].ubWeaponType)
		{
		case GUN_PISTOL:
...
void TacticalActor::BeginSoldierClimbFence( void )
{
	INT8							bDirection;
...
void TacticalActor::BeginSoldierClimbWindow( void )
{
	INT8	bDirection;
...
void TacticalActor::BeginSoldierClimbWall( void )
{
	INT8 bNewDirection;
	SoldierID ubWhoIsThere;
...
void TacticalActor::BeginSoldierClimbWallUp( void )
{
	INT8 bNewDirection;
	SoldierID ubWhoIsThere;
...
void TacticalActor::BeginSoldierClimbDownRoof(void)
{
	INT8 bNewDirection;
	SoldierID ubWhoIsThere;
...
/*
void BeginSoldierClimbDownRoof( TacticalActor *pSoldier )
{
INT8							bNewDirection;
UINT8	ubWhoIsThere;
...
void TacticalActor::EVENT_SoldierBeginGiveItem( void )
{
	TacticalActor *pTSoldier;
...
void TacticalActor::EVENT_SoldierBeginBladeAttack( INT32 sGridNo, UINT8 ubDirection )
{
	TacticalActor *pTSoldier;
	//UINT32 uiMercFlags;
	...
	if (this->position().direction() != ubDirection)
	{
		...
		if (this->animationActivity().turningFromProneMode())
			this->animationActivity().turningFromProneMode() = TURNING_FROM_PRONE_FOR_PUNCH_OR_STAB;
		usForceAnimState = INVALID_ANIMATION;
	}
	// CHANGE TO ANIMATION
	...
	else if ( this->identity().bodyType() == BLOODCAT )
	{
		// Check if it's a claws or teeth...
		if ( this->inventory()[HANDPOS].usItem == BLOODCAT_CLAW_ATTACK )
		{
	...
	else
	{
		...
		if ( usSoldierIndex != NOBODY )
		{
			...
			if (pTSoldier)
			{
				pTSoldier->featureFlags().secondaryFlags() &= ~SOLDIER_BACK_ATTACK;
				pTSoldier->featureFlags().secondaryFlags() &= ~SOLDIER_SNEAK_ATTACK;
				...
				if (pTSoldier->awareness().opponentKnowledge()[this->identity().id()] != SEEN_CURRENTLY &&
					pTSoldier->awareness().opponentKnowledge()[this->identity().id()] != SEEN_THIS_TURN &&
					pTSoldier->awareness().opponentKnowledge()[this->identity().id()] != HEARD_THIS_TURN)
				{
					pTSoldier->featureFlags().secondaryFlags() |= SOLDIER_SNEAK_ATTACK;
				}
			}
			...
			case ANIM_CROUCH:
				...
				if ( this->attackSelection().weaponMode() == WM_ATTACHED_BAYONET && gAnimControl[this->animationPlayback().state()].ubEndHeight == ANIM_STAND)
				{
					...
					if (this->animationPlayback().state() == RUNNING || this->animationPlayback().state() == RUNNING_W_PISTOL)
						this->featureFlags().secondaryFlags() |= SOLDIER_BAYONET_RUNBONUS;
				...
				else if (gGameExternalOptions.fEnhancedCloseCombatSystem && (this->aiPlanning().aimTime() > 0))
				{
...
void TacticalActor::EVENT_SoldierBeginPunchAttack( INT32 sGridNo, UINT8 ubDirection )
{
	...
	else
	{
		...
		if ( this->IsZombie( ) && IsAnimationValidForBodyType( this, PUNCH ) == FALSE )
		{
			if ( gGameExternalOptions.fZombieExplodingCivs )
			{
				...
				IgniteExplosion( this->identity().id(), this->position().worldXInt(), this->position().worldYInt(), (INT16)(gpWorldLevelData[this->position().gridNo()].sHeight), this->position().gridNo(), 136, this->position().level() );
			}
			else
			{
				...
				if ( Random( 100 ) > 30 )
				{
					...
					pTSoldier->SoldierTakeDamage( 0, damage, breathdamage, TAKE_DAMAGE_HANDTOHAND, this->identity().id(), pTSoldier->position().gridNo(), 0, TRUE );
		...
		else
		{
			...
			case ANIM_CROUCH:
				...
				if ( !ItemIsCrowbar(usItem) )
				{
					...
					if ( gGameExternalOptions.fEnhancedCloseCombatSystem && (this->aiPlanning().aimTime() > 0) )
					{
...
void TacticalActor::EVENT_SoldierBeginKnifeThrowAttack( INT32 sGridNo, UINT8 ubDirection )
{
	// Increment the number of people busy doing stuff because of an attack
	//if ( IsJa2TacticalTurnBasedCombat() )
	//{
	...
	if ( this->identity().bodyType() == BIGMALE && (DoesMercHavePersonality( this, CHAR_TRAIT_SHOWOFF ) || (HAS_SKILL_TRAIT( this, THROWING_NT ) && gGameOptions.fNewTraitSystem) || (HAS_SKILL_TRAIT( this, THROWING_OT ) && !gGameOptions.fNewTraitSystem)))
	{
	...
	//dnl ch70 160913 ugly but fast fix for not charging turning APs as there is no fire ready animation for throwing knives and I don't want to break TacticalActor just for that
...
void TacticalActor::EVENT_SoldierBeginDropBomb( )
{
	// Increment the number of people busy doing stuff because of an attack
...
void TacticalActor::EVENT_SoldierDefuseTripwire( INT32 sGridNo, INT32 sItem )
{
...
void TacticalActor::EVENT_SoldierBeginUseDetonator( void )
{
	// Increment the number of people busy doing stuff because of an attack
	switch ( gAnimControl[this->animationPlayback().state()].ubHeight )
	{
...
UINT32 TacticalActor::SoldierDressWound( TacticalActor *pVictim, INT16 sKitPts, INT16 sStatus )
{
	...
	if ( (pVictim->vitals().bleeding() < 1 && pVictim->vitals().health() >= OKLIFE) && !(pVictim->vitals().hasHealableInjury() && this->vitals().isUndergoingSurgery()) )
	{
	...
	if (gGameExternalOptions.fDynamicOpinions)
	{
		AddOpinionEvent(pVictim->identity().profile(), this->identity().profile(), OPINIONEVENT_BANDAGED);
	}
	...
	if ( pVictim->vitals().hasHealableInjury() && this->vitals().isUndergoingSurgery() && (this->identity().id() != pVictim->identity().id() || (gGameExternalOptions.fEnemyMedicsHealSelf && this->roster().team() == ENEMY_TEAM))
		 && gGameOptions.fNewTraitSystem && (NUM_SKILL_TRAITS( this, DOCTOR_NT ) >= gSkillTraitValues.ubDONumberTraitsNeededForSurgery)
		 && (ItemIsMedicalKit(this->inventory()[HANDPOS].usItem) || this->roster().team() == ENEMY_TEAM) )
	{
		fOnSurgery = TRUE;
	}
	...
	if (ItemIsMedicalKit(this->inventory()[HANDPOS].usItem) && !(fOnSurgery) ) // added check
	{
	...
	if ( gAnimControl[this->animationPlayback().state()].ubEndHeight == ANIM_PRONE )
	{
		...
		if ( this->identity().id() == pVictim->identity().id() )
	...
	if (ItemIsMedicalKit(this->inventory()[HANDPOS].usItem))
	{
		if ( fOnSurgery )
	...
	if ( fOnSurgery && gGameOptions.fNewTraitSystem ) // double check for new traits
	{
		...
		{
			if ( this->featureFlags().secondaryFlags() & SOLDIER_SURGERY_BOOSTED )
				usLifeReturned = pVictim->vitals().healableInjury() * ( gSkillTraitValues.ubDOSurgeryHealPercentBase + gSkillTraitValues.ubDOSurgeryHealPercentBloodbag + gSkillTraitValues.ubDOSurgeryHealPercentOnTop * NUM_SKILL_TRAITS( this, DOCTOR_NT ) ) / 100;
			else
				usLifeReturned = pVictim->vitals().healableInjury() * (gSkillTraitValues.ubDOSurgeryHealPercentBase + gSkillTraitValues.ubDOSurgeryHealPercentOnTop * NUM_SKILL_TRAITS( this, DOCTOR_NT )) / 100;
			...
			if ( pVictim->identity().profile() != NO_PROFILE && usLifeReturned >= 100 )
				gMercProfiles[pVictim->identity().profile()].records.usTimesSurgeryUndergoed++;

			// add to record - another surgery made
			if ( this->identity().profile() != NO_PROFILE && usLifeReturned >= 100 )
				gMercProfiles[this->identity().profile()].records.usSurgeriesMade++;
		}
		else
		{
			if ( this->featureFlags().secondaryFlags() & SOLDIER_SURGERY_BOOSTED )
	...
	if (ItemIsMedicalKit(this->inventory()[HANDPOS].usItem) && !(fOnSurgery) )	// using the GOOD medic stuff
	{
	...
	if ( bInitialBleeding > 1 && pVictim->vitals().bleeding() == 0 && this->identity().profile() != NO_PROFILE )
		gMercProfiles[this->identity().profile()].records.usMercsBandaged++;
...
void TacticalActor::ReLoadSoldierAnimationDueToHandItemChange( UINT16 usOldItem, UINT16 usNewItem )
{
	...
	if ( Weapon[usNewItem].ubShotsPerBurst == 0 && !Weapon[this->inventory()[HANDPOS].usItem].NoSemiAuto )
	{
	...
	if ( HasAttachmentOfClass( &(this->inventory()[HANDPOS]), AC_RIFLEGRENADE ) )
	{
		OBJECTTYPE* pRifleGrenadeDeviceObj = FindAttachment_GrenadeLauncher( &(this->inventory()[HANDPOS]) );

		if ( pRifleGrenadeDeviceObj && FindLaunchableAttachment( &(this->inventory()[HANDPOS]), pRifleGrenadeDeviceObj->usItem ) )
	...
	if ( usNewItem != NOTHING )
	{
		if ( Item[usNewItem].usItemClass == IC_GUN )
		{
			...
			if ( (Item[inventory()[HANDPOS].usItem].usItemClass & IC_WEAPON) && (Item[inventory()[SECONDHANDPOS].usItem].usItemClass & IC_WEAPON) )
			{
				std::map<INT8, OBJECTTYPE*> ObjList;
				GetScopeLists( this, &inventory()[HANDPOS], ObjList );
...
void TacticalActor::PrintFoodDesc( CHAR16* apStr, BOOLEAN fFullDesc )
{
	if ( !UsingFoodSystem() )
		return;
...
void TacticalActor::PrintSleepDesc( CHAR16* apStr )
{
	// only for people
	if ( this->status().flags() & SOLDIER_VEHICLE )
		return;
...
FLOAT		TacticalActor::GetBurialPoints( UINT16* apCorpses )
{
	if ( this->vitals().health() < OKLIFE || this->deployment().sectorZ() || ( this->featureFlags().primaryFlags() & SOLDIER_POW ) )
		return 0.0f;
	...
	{
		// ... if Item exists and is canteen (that can have drink points) ...
		if ( inventory()[bLoop].exists() == true && Item[inventory()[bLoop].usItem].usBurialModifier )
		{
			OBJECTTYPE * pObj = &( this->inventory()[bLoop] );							// ... get pointer for this item ...
			...
			if ( pObj != NULL )													// ... if pointer is not obviously useless ...
			{
				...
				{
					FLOAT modifier = 1.0f + ( Item[inventory()[bLoop].usItem].usBurialModifier * ( *pObj )[i]->data.objectStatus ) / 10000.0f;
...
INT8	TacticalActor::GetSleepBreathRegeneration( )
{
...
FLOAT	TacticalActor::GetConstructionPoints( )
{
	if ( this->vitals().health() < OKLIFE || this->assignment().isAsleep() || this->collapseState().tactical() || (this->featureFlags().primaryFlags() & SOLDIER_POW) )
		return 0;
...
UINT32		TacticalActor::GetAdministrationPoints()
{
	if ( this->vitals().health() < OKLIFE || this->deployment().sectorZ() || ( this->featureFlags().primaryFlags() & SOLDIER_POW ) )
		return 0;
	...
	for ( INT8 bLoop = 0; bLoop < invsize; ++bLoop )						// ... for all items in our inventory ...
	{
		if ( inventory()[bLoop].exists() == true && Item[inventory()[bLoop].usItem].usAdministrationModifier )
		{
			OBJECTTYPE * pObj = &( this->inventory()[bLoop] );							// ... get pointer for this item ...
			...
			if ( pObj != NULL )													// ... if pointer is not obviously useless ...
			{
				for ( INT16 i = 0; i < pObj->ubNumberOfObjects; ++i )
				{
					FLOAT modifier = 1.0f + ( Item[inventory()[bLoop].usItem].usAdministrationModifier * ( *pObj )[i]->data.objectStatus ) / 10000.0f;
	...
	FLOAT approachmax = max( gMercProfiles[this->identity().profile()].usApproachFactor[0], max( gMercProfiles[this->identity().profile()].usApproachFactor[1], gMercProfiles[this->identity().profile()].usApproachFactor[2] ) );
...
FLOAT		TacticalActor::GetAdministrationModifier()
{
...
UINT32		TacticalActor::GetExplorationPoints()
{
	if ( this->vitals().health() < OKLIFE || ( this->featureFlags().primaryFlags() & SOLDIER_POW ) )
		return 0;
...
void TacticalActor::EVENT_SoldierBeginCutFence( INT32 sGridNo, UINT8 ubDirection )
{
...
void TacticalActor::EVENT_SoldierBeginRepair( INT32 sGridNo, UINT8 ubDirection )
{
	INT8 bRepairItem;
	UINT16	ubID;
...
void TacticalActor::EVENT_SoldierBeginRefuel( INT32 sGridNo, UINT8 ubDirection )
{
	INT8 bRefuelItem;
	UINT16	ubID;
...
void TacticalActor::EVENT_SoldierBeginTakeBlood( INT32 sGridNo, UINT8 ubDirection )
{
	ROTTING_CORPSE *pCorpse;
...
void TacticalActor::EVENT_SoldierBeginAttachCan( INT32 sGridNo, UINT8 ubDirection )
{
	STRUCTURE *			pStructure;
	...
	if ( !ItemIsCanAndString(this->inventory()[HANDPOS].usItem))
	{
		return;
	}
	...
	// Remove item...
	this->inventory()[HANDPOS].RemoveObjectsFromStack( 1 );
	fInterfacePanelDirty = DIRTYLEVEL2;
...
void TacticalActor::EVENT_SoldierBuildStructure( INT32 sGridNo, UINT8 ubDirection )
{
	BOOLEAN fSuccess = FALSE;
	...
	if ( pObj && pObj->exists( ) && ( IsStructureConstructItem( this->inventory()[HANDPOS].usItem, sGridNo, this ) || IsStructureDeconstructItem( this->inventory()[HANDPOS].usItem, sGridNo, this )) )
	{
...
void TacticalActor::EVENT_SoldierHandcuffPerson( INT32 sGridNo, UINT8 ubDirection )
{
	...
	if ( pSoldier != nullptr && pSoldier->CanBeCaptured( ) )
	{
		...
		if ( success )
		{
			...
			if ( pSoldier->inventory()[HANDPOS].exists( ) == true && !(pSoldier->inventory()[HANDPOS].fFlags & OBJECT_UNDROPPABLE) )
			{
				// ATE: if our guy, make visible....
				INT8 bVisible = (pSoldier->roster().team() == gbPlayerNum) ? 1 : 0;
				UINT16 itemflags = (pSoldier->roster().team() == ENEMY_TEAM) ? 0 : WORLD_ITEM_DROPPED_FROM_ENEMY;

				if ( UsingNewAttachmentSystem( ) == true )
					ReduceAttachmentsOnGunForNonPlayerChars( pSoldier, &(pSoldier->inventory()[HANDPOS]) );

				AddItemToPool( pSoldier->position().gridNo(), &(pSoldier->inventory()[HANDPOS]), bVisible, pSoldier->position().level(), itemflags, -1 ); //Madd: added usItemFlags to function arguments
				DeleteObj( &(pSoldier->inventory()[HANDPOS]) );
			}

			if ( pSoldier->inventory()[SECONDHANDPOS].exists( ) == true && !(pSoldier->inventory()[SECONDHANDPOS].fFlags & OBJECT_UNDROPPABLE) )
			{
				// ATE: if our guy, make visible....
				INT8 bVisible = (pSoldier->roster().team() == gbPlayerNum) ? 1 : 0;
				UINT16 itemflags = (pSoldier->roster().team() == ENEMY_TEAM) ? 0 : WORLD_ITEM_DROPPED_FROM_ENEMY;
				...
				if ( UsingNewAttachmentSystem( ) == true )
					ReduceAttachmentsOnGunForNonPlayerChars( pSoldier, &(pSoldier->inventory()[SECONDHANDPOS]) );

				AddItemToPool( pSoldier->position().gridNo(), &(pSoldier->inventory()[SECONDHANDPOS]), bVisible, pSoldier->position().level(), itemflags, -1 ); //Madd: added usItemFlags to function arguments
				DeleteObj( &(pSoldier->inventory()[SECONDHANDPOS]) );
			}

			// move handcuffs to his hands
			if ( HasItemFlag( (&(this->inventory()[HANDPOS]))->usItem, HANDCUFFS ) )
			{
				// stack handcuffs (like a stack of binders) simply gets used up a bit
				if ( Item[(&(this->inventory()[HANDPOS]))->usItem].usItemClass == IC_KIT )
				{
					UseKitPoints( &(this->inventory()[HANDPOS]), 10, pSoldier );
				}
				else
				{
					AutoPlaceObject( pSoldier, &(this->inventory()[HANDPOS]), FALSE );
					DeleteObj( &(this->inventory()[HANDPOS]) );
				}
			}
...
void TacticalActor::EVENT_SoldierApplyItemToPerson( INT32 sGridNo, UINT8 ubDirection )
{
	SoldierID ubPerson = WhoIsThere2( sGridNo, this->position().level() );
	TacticalActor* pSoldier =
		GetJa2SoldierRepository().resolve(
	...
	if ( pSoldier != nullptr )
	{
		...
		if ( pSoldier && pObj->exists( ) )
		{
			...
			if ( ItemCanBeAppliedToOthers( usItem ) )
			{
				...
				if ( this->roster().side() != pSoldier->roster().side() && !pSoldier->collapseState().tactical() )
				{
					...
					UINT32 defendervalue = 100 + 3 * EffectiveExpLevel( pSoldier ) + 100 * (pSoldier->aiBehavior().alertStatus() - 1);
				...
				if ( success )
				{
					if (ItemIsGasmask(usItem))
					{
						...
						{
							if ( !(pSoldier->inventory()[HEAD1POS]).exists( ) )
							{
								success = PlaceObject( pSoldier, HEAD1POS, pObj );
							}
							else if ( !(pSoldier->inventory()[HEAD2POS]).exists( ) )
							{
							...
							else
							{
								...
								AddItemToPool( pSoldier->position().gridNo(), &(pSoldier->inventory()[HEAD2POS]), 1, pSoldier->position().level(), 0, -1 );
...
void TacticalActor::EVENT_SoldierTakeBloodFromPerson( INT32 sGridNo, UINT8 ubDirection )
{
	SoldierID ubPerson = WhoIsThere2( sGridNo, this->position().level() );
	TacticalActor* pSoldier =
		GetJa2SoldierRepository().resolve(
	...
	if ( pSoldier != nullptr && ubPerson != this->identity().id() )
	{
		// we found someone
		OBJECTTYPE* pObj = &( this->inventory()[HANDPOS] );
		...
		if ( pSoldier && pObj->exists() && HasItemFlag( pObj->usItem, EMPTY_BLOOD_BAG ) && pSoldier->IsValidBloodDonor() )
		{
			...
			if ( HasItemFlag( bloodbagitem, BLOOD_BAG ) || GetFirstItemWithFlag( &bloodbagitem, BLOOD_BAG ) )
			{
				...
				INT32 healableinjury_tmp = pSoldier->vitals().healableInjury();
...
void TacticalActor::EVENT_SoldierApplySplintToPerson( INT32 sGridNo, UINT8 ubDirection )
{
	SoldierID ubPerson = WhoIsThere2( sGridNo, this->position().level() );
	TacticalActor* pSoldier =
		GetJa2SoldierRepository().resolve(
...
void TacticalActor::EVENT_SoldierInteractiveAction( INT32 sGridNo, UINT16 usActionType )
{
	DoInteractiveAction( sGridNo, this );
}

void TacticalActor::EVENT_SoldierBeginReloadRobot( INT32 sGridNo, UINT8 ubDirection, UINT8 ubMercSlot )
{
	...
	if ( robot != nullptr && robot->status().flags() & SOLDIER_ROBOT )
	{
		// CHANGE DIRECTION AND GOTO ANIMATION NOW
		if (this->position().direction() != ubDirection)
		{
...
BOOLEAN TacticalActor::PlayerSoldierStartTalking( SoldierID ubTargetID, BOOLEAN fValidate )
{
	...
	if ( GetCivType( pTSoldier ) != CIV_TYPE_NA )
	{
		...
		else
		{
			...
			else if ( pTSoldier->interaction().isNonNpcTrader() )
			{
				...
				if ( !pTSoldier->aiBehavior().neutral() && !(this->featureFlags().primaryFlags() & SOLDIER_COVERT_SOLDIER) )
				{
				...
				// however, if we are covert as a soldier, this check does not apply - merchants know better than do defy the army
				else if ( ubTownID != BLANK_SECTOR && gTownLoyalty[ubTownID].ubRating < armsDealerInfo[pTSoldier->interaction().nonNpcTraderId()].nonprofile_loyaltyrequired && !(this->featureFlags().primaryFlags() & SOLDIER_COVERT_SOLDIER) )
				{
				...
				{
					DeductPoints( this, sAPCost, 0, UNTRIGGERED_INTERRUPT );
					EnterShopKeeperInterfaceScreen_NonNPC( pTSoldier->interaction().nonNpcTraderId(), pTSoldier->identity().id() );
				}
	...
	if ( pTSoldier->identity().profile() != NO_PROFILE && pTSoldier->employment().mercenaryType() == MERC_TYPE__EPC )
	{
		return(InitiateConversation( pTSoldier, this, APPROACH_EPC_WHO_IS_RECRUITED, 0 ));
		//Converse( pTSoldier->identity().profile(), this->ubProfile, APPROACH_EPC_WHO_IS_RECRUITED, 0 );
	}
	else if ( pTSoldier->aiBehavior().neutral() )
	{
		if ( pTSoldier->IsAssassin( ) )
		{
			...
			CancelAIAction( pTSoldier, TRUE );
			AddToShouldBecomeHostileOrSayQuoteList( pTSoldier->identity().id() );
		}
		else
		{
			// Start talking!
...
void BeginSoldierClimbWallUp( TacticalActor *pSoldier )
{
	INT8			bNewDirection;
	SoldierID	ubWhoIsThere;
...
void TacticalActor::BreakWindow(void)
{
	if (this->inventory()[HANDPOS].exists() &&
		this->inventory()[HANDPOS][0]->data.objectStatus >= USABLE &&
		(ItemIsCrowbar(this->inventory()[HANDPOS].usItem) &&	Item[this->inventory()[HANDPOS].usItem].usItemClass & (IC_PUNCH) ||
		Item[this->inventory()[HANDPOS].usItem].usItemClass & IC_GUN && ItemIsTwoHanded(this->inventory()[HANDPOS].usItem) && ItemIsMetal(this->inventory()[HANDPOS].usItem) ))
	{
		this->attackSelection().weapon() = this->inventory()[HANDPOS].usItem;
		this->aiPlanning().action() = AI_ACTION_KNIFE_STAB;
		this->aiPlanning().actionData() = this->position().gridNo();
		this->pendingAction().clearAction();
...
UINT16 NumberOfDamagedStats( TacticalActor * pSoldier )
{
	...
	for ( UINT8 cnt = 0; cnt < NUM_DAMAGABLE_STATS; ++cnt )
	{
		if ( pSoldier->vitals().criticalStatDamage()[cnt] > 0 )
			ubTotalStatsDamaged += pSoldier->vitals().criticalStatDamage()[cnt];
	}
		...
		if ( pSoldier->vitals().criticalStatDamage()[cnt] > 0 )
		{
			...
			if ( usStatIncreasement > 0 )
			{
				...
				case DAMAGED_STAT_HEALTH:
					...
					pSoldier->vitals().healableInjury() -= (usStatIncreasement * 100); // don't forget the healable injury
					...
					gMercProfiles[pSoldier->identity().profile()].bLifeMax = pSoldier->vitals().maximumHealth(); // update profile
					break;
				case DAMAGED_STAT_DEXTERITY:
					...
					gMercProfiles[pSoldier->identity().profile()].bDexterity = pSoldier->statistics().dexterity(); // update profile
					break;
				case DAMAGED_STAT_AGILITY:
					...
					gMercProfiles[pSoldier->identity().profile()].bAgility = pSoldier->statistics().agility(); // update profile
					break;
				case DAMAGED_STAT_STRENGTH:
					...
					gMercProfiles[pSoldier->identity().profile()].bStrength = pSoldier->statistics().strength(); // update profile
					break;
				case DAMAGED_STAT_WISDOM:
					...
					gMercProfiles[pSoldier->identity().profile()].bWisdom = pSoldier->statistics().wisdom(); // update profile
					break;
				}
...
UINT32 VirtualSoldierDressWound( TacticalActor *pSoldier, TacticalActor *pVictim, OBJECTTYPE *pKit, INT16 sKitPts, INT16 sStatus, BOOLEAN fOnSurgery )
{
	...
	if (ItemIsMedicalKit(pSoldier->inventory()[0].usItem) && !(fOnSurgery) )		// using the GOOD medic stuff
		uiPossible += (uiPossible / 2);			// add extra 50 %
	...
	if ( fOnSurgery && gGameOptions.fNewTraitSystem ) // double check for new traits
	{
		...
		if ( bPtsLeft >= (pVictim->vitals().healableInjury() / 100) )
		{
			...
			// add to record - another surgery undergoed
			if ( pVictim->identity().profile() != NO_PROFILE && iLifeReturned >= 100 )
				gMercProfiles[pVictim->identity().profile()].records.usTimesSurgeryUndergoed++;

			// add to record - another surgery made
			if ( pSoldier->identity().profile() != NO_PROFILE && iLifeReturned >= 100 )
				gMercProfiles[pSoldier->identity().profile()].records.usSurgeriesMade++;
		}
		else
		{
			if ( pSoldier->featureFlags().secondaryFlags() & SOLDIER_SURGERY_BOOSTED )
	...
	// merc records - bandaging
	if ( bInitialBleeding > 1 && pVictim->vitals().bleeding() == 0 && pSoldier->identity().profile() != NO_PROFILE )
		gMercProfiles[pSoldier->identity().profile()].records.usMercsBandaged++;
...
{
	if ( !pRecruiter || pRecruiter->roster().team() != OUR_TEAM )
		return;
	
	// potential recruit must be a civilain NPC of no other affilation, no kids
	if ( !pTarget || pTarget->roster().team() != CIV_TEAM || pTarget->identity().profile() != NO_PROFILE || pTarget->roster().civilianGroup() != NON_CIV_GROUP || pTarget->identity().bodyType() > DRESSCIV )
		return;
	...
	if ( pTarget->featureFlags().secondaryFlags() & SOLDIER_POTENTIAL_VOLUNTEER )
	{
		...
		if ( rating > 70.0 )
		{
			...
			pTarget->featureFlags().secondaryFlags() &= ~SOLDIER_POTENTIAL_VOLUNTEER;

			pTarget->roster().civilianGroup() = VOLUNTEER_CIV_GROUP;
