...
INT32 ArmourPercent( TacticalActor * pSoldier )
{
	...
	if (pSoldier->inventory()[VESTPOS].exists() == true)
	{
		iVest = EffectiveArmour( &(pSoldier->inventory()[VESTPOS]) );
	...
	if (pSoldier->inventory()[HELMETPOS].exists() == true)
	{
		iHelmet = EffectiveArmour( &(pSoldier->inventory()[HELMETPOS]) );
	...
	if (pSoldier->inventory()[LEGPOS].exists() == true)
	{
		iLeg = EffectiveArmour( &(pSoldier->inventory()[LEGPOS]) );
	...
	INT32 iVestPack;
	if (pSoldier->inventory()[VESTPOCKPOS].exists() == true)
	{
		iVestPack = EffectiveArmourLBE( &(pSoldier->inventory()[VESTPOCKPOS]) );
		iDivideValue = ( ( Armour[ Item[ SPECTRA_VEST_18 ].ubClassIndex ].ubProtection * Armour[ Item[ SPECTRA_VEST_18 ].ubClassIndex ].ubCoverage ) + ( Armour[ Item[ CERAMIC_PLATES ].ubClassIndex ].ubProtection * Armour[ Item[ CERAMIC_PLATES ].ubClassIndex ].ubCoverage ) );
...
INT8 ArmourVersusExplosivesPercent( TacticalActor * pSoldier )
{
	...
	if (pSoldier->inventory()[VESTPOS].exists() == true)
	{
		iVest = ExplosiveEffectiveArmour( &(pSoldier->inventory()[VESTPOS]) );
		// convert to % of best; ignoring bug-treated stuff
	...
	if (pSoldier->inventory()[HELMETPOS].exists() == true)
	{
		iHelmet = ExplosiveEffectiveArmour( &(pSoldier->inventory()[HELMETPOS]) );
		// convert to % of best; ignoring bug-treated stuff
	...
	if (pSoldier->inventory()[LEGPOS].exists() == true)
	{
		iLeg = ExplosiveEffectiveArmour( &(pSoldier->inventory()[LEGPOS]) );
		// convert to % of best; ignoring bug-treated stuff
	...
	if (pSoldier->inventory()[VESTPOCKPOS].exists() == true)
	{
		iVestPack = ExplosiveEffectiveArmourLBE( &(pSoldier->inventory()[VESTPOCKPOS]) );
		// convert to % of best; ignoring bug-treated stuff
...
INT16 ArmourVersusFirePercent( TacticalActor * pSoldier )
{
	...
	for ( int i = BODYPOSSTART; i < BODYPOSFINAL; ++i )
	{
		...
		{
			val += FireEffectiveArmour( &( pSoldier->inventory()[i] ) );
		}
	}
...
{ 
	...
	if (pSoldier->status().flags() & SOLDIER_PC || gGameExternalOptions.fEnemyJams)
	{ 
		...
		{ 
			...
			{ 
				...
				{ 
					...
					// Deduct AMMO! 
					DeductAmmo( pSoldier, pSoldier->attackSelection().hand() );
			...
			{ 
				...
				if(EnoughPoints(pSoldier, APBPConstants[AP_UNJAM], APBPConstants[BP_UNJAM], FALSE))
				{
					...
					if ( Weapon[pSoldier->inventory()[pSoldier->attackSelection().hand()].usItem].EasyUnjam )
						bChanceMod = 100;
					else
					...
					if (iResult > 0 || !(pSoldier->status().flags() & SOLDIER_PC))
					{ 
...
BOOLEAN	OKFireWeapon( TacticalActor *pSoldier )
{
	...
	{
		//if ( Weapon[pSoldier->inventory()[pSoldier->attackSelection().hand()].usItem].NoSemiAuto ) // Madd: no autofire w/second hand (yet)
		//{
		//	return( FALSE );
		//}
...
BOOLEAN FireWeapon( TacticalActor *pSoldier , INT32 sTargetGridNo )
{
	...
	if ( pSoldier->attackSelection().weaponMode() == WM_ATTACHED_BAYONET )
	...
	switch( usItemClass )
	{
		...
		case IC_LAUNCHER:
			if (ItemIsRocketLauncher(pSoldier->attackSelection().weapon())){
				UseGunWrapper( pSoldier, sTargetGridNo );
			} else {
				// ATE: PAtch up - bookkeeping for spreading done out of whak
				if ( pSoldier->fireControl().spreadIndex())
...
void GetTargetWorldPositions( TacticalActor *pSoldier, INT32 sTargetGridNo, FLOAT *pdXPos, FLOAT *pdYPos, FLOAT *pdZPos )
{
	...
	{
		// SAVE OPP ID
		pSoldier->targeting().engageOpponent( pTargetSoldier->identity().id() );
		ConvertGridNoToCenterCellXY(pTargetSoldier->position().gridNo(), &sX, &sY);
		dTargetX = (FLOAT) sX;
		dTargetY = (FLOAT) sY;
		if (pSoldier->attackSelection().shotLocation() == AIM_SHOT_RANDOM)
		{
			...
			if ( pSoldier->attackSelection().shotLocation() != AIM_SHOT_HEAD )
			{
				UINT32 uiChanceToGetThrough = SoldierToSoldierBodyPartChanceToGetThrough( pSoldier, pTargetSoldier, pSoldier->attackSelection().shotLocation() );
...
BOOLEAN UseGunNCTH( TacticalActor *pSoldier , INT32 sTargetGridNo )
{
	...
	sAPCost = CalcTotalAPsToAttack( pSoldier, sTargetGridNo, FALSE, pSoldier->aiPlanning().aimTime() );
	...
	{
	    uiMuzzleSway = 100 - CalcThrownChanceToHit( pSoldier, sTargetGridNo, pSoldier->aiPlanning().aimTime(), pSoldier->attackSelection().shotLocation() );
	}
	else
	{
		uiMuzzleSway = 100 - CalcChanceToHitGun( pSoldier, sTargetGridNo, pSoldier->aiPlanning().aimTime(), pSoldier->attackSelection().shotLocation() );
	}
...
//ddd}
	...
	if ( Item[ usUBItem ].usItemClass != IC_THROWING_KNIFE )
	{
		...
		if ( ( ( gTacticalStatus.uiFlags & REALTIME ) || !( IsJa2TacticalCombatActive() ) ) )
		{
			...
			//pSoldier->fireControl().reloading()		= TRUE;
	...
	if (ItemIsRocketLauncher(usUBItem))
	{
		if ( WillExplosiveWeaponFail( pSoldier, pObjHand ) )
		{
			...
			if ( ItemIsSingleShotRocketLauncher(usUBItem)  )
			{
				...
				{
					IgniteExplosion( pSoldier->identity().id(), sX, sY, 0, pSoldier->position().gridNo(), Item[usUBItem].usBuddyItem, pSoldier->position().level() );
				}
				else
				{
					IgniteExplosion( pSoldier->identity().id(), sX, sY, 0, pSoldier->position().gridNo(), C1, pSoldier->position().level() );
				}
			}
			else
			{
				...
				IgniteExplosion( pSoldier->identity().id(), sX, sY, 0, pSoldier->position().gridNo(), (*pObjHand)[0]->data.gun.usGunAmmoItem, pSoldier->position().level() );
				pSoldier->inventory()[pSoldier->attackSelection().hand() ][0]->data.gun.usGunAmmoItem = NONE;
			}
	...
	if ( Item[ usUBItem ].usItemClass != IC_THROWING_KNIFE )
	{
		...
		if ( PTR_OURTEAM && pSoldier->targeting().targetId() != NOBODY &&
			...
			( SoldierToSoldierBodyPartChanceToGetThrough(
				...
				pSoldier->attackSelection().shotLocation() ) > 0 ) )
		{
			...
			if ( pAttackTarget->identity().bodyType() == COW ||
				pAttackTarget->identity().bodyType() == CROW )
			{
				dExpGain /= 2;
			}
			else if ( pAttackTarget->status().flags() &
					SOLDIER_VEHICLE ||
			...
			if (pSoldier->aiPlanning().aimTime())
			{
	...
	if ( Item[ usItemNum ].usItemClass == IC_THROWING_KNIFE )
	{
		...
		pSoldier->inventory()[ HANDPOS ].RemoveObjectsFromStack(1);
		//shadooow: if we have another throwing knife in second hand, move it to main hand
		if (pSoldier->inventory()[SECONDHANDPOS].exists() && (Item[pSoldier->inventory()[SECONDHANDPOS].usItem].usItemClass & IC_THROWING_KNIFE))
		{
...
//				GetJa2PendingTacticalCombatActions()++;
	...
	{
		if ( pSoldier->roster().team() == OUR_TEAM || pSoldier->roster().team() == MILITIA_TEAM )
	...
	//if ( Item[usItemNum].usItemClass == IC_GUN )
	//{
	//	OBJECTTYPE *pGun = &(pSoldier->inventory()[pSoldier->attackSelection().hand()]);
	...
	if ( UsingNewCTHSystem() == true )
	{
		UINT16 usBaseChance = gGameCTHConstants.BASIC_RELIABILITY_ODDS;
		FLOAT dReliabilityRatio = 3.0f * ( (FLOAT)usBaseChance / (FLOAT)gItemSettings.usBasicDeprecateChance ); // Compare original odds to new odds.
		depreciatetest = ( usBaseChance + (INT16)( dReliabilityRatio * GetReliability( &( pSoldier->inventory()[pSoldier->attackSelection().hand()] ) ) - iOverheatReliabilityMalus ) ) / dirtincreasefactor;
	}
	else
	{
	...
	if ( !PreRandom( uiDepreciateTest ) && ( (*pObjAttHand)[0]->data.objectStatus > 1) )
	{
		if (AM_A_ROBOT(pSoldier) && HasItemFlag(pSoldier->inventory()[ROBOT_UTILITY_SLOT].usItem, CLEANING_KIT))
		{
			OBJECTTYPE* pRobotCleaningKit = &pSoldier->inventory()[ROBOT_UTILITY_SLOT];
	...
	if ( pSoldier->perception().monsterSmell() > 0 && Random( 2 ) == 0 )
	{
...
//	CHAR8		zBurstString[512];
	...
	if ( Item[ usUBItem ].usItemClass == IC_THROWING_KNIFE )
	{
	  uiHitChance = CalcThrownChanceToHit( pSoldier, sTargetGridNo, pSoldier->aiPlanning().aimTime(), pSoldier->attackSelection().shotLocation() );
	}
	else
	{
		fCalculateCTHDuringGunfire = TRUE;
		uiHitChance = CalcChanceToHitGun( pSoldier, sTargetGridNo, pSoldier->aiPlanning().aimTime(), pSoldier->attackSelection().shotLocation() );
		//CHRISL:
	...
	if ( Item[ usUBItem ].usItemClass != IC_THROWING_KNIFE )
	{
				...
				pSoldier->attackSelection().shotLocation() ) > 0 ) )
		{
			if ( fGonnaHit )
			{
				...
				if ( pSoldier->aiPlanning().aimTime() && !pSoldier->fireControl().burstCounter() )
				{
					// gain extra exp for aiming, up to the amount from
					// the difficulty of the shot
					usExpGain += __min( pSoldier->aiPlanning().aimTime(), usExpGain );
				}
		...
		if (!CREATURE_OR_BLOODCAT( pSoldier ) )
		{
			...
			//switch ( pSoldier->inventory()[ pSoldier->attackSelection().hand() ][0]->data.gun.ubGunAmmoType )
			//{
			//	case AMMO_BUCKSHOT:
			//		fBuckshot = TRUE;
			//		break;
		...
		if (PTR_OURTEAM && pSoldier->targeting().targetId() != NOBODY)
		{
			if ( fGonnaHit )
			{
				...
				if (pSoldier->aiPlanning().aimTime())
				{
					...
					usExpGain += ( 2 * __min( pSoldier->aiPlanning().aimTime(), usExpGain ) );
				}
	...
	if (ItemIsRocketLauncher(usUBItem))
	{
		if ( WillExplosiveWeaponFail( pSoldier, pObjUsed ) )
		{
			...
			else
			{
				...
				IgniteExplosion( pSoldier->identity().id(), sX, sY, 0, pSoldier->position().gridNo(), (*pObjUsed)[0]->data.gun.usGunAmmoItem, pSoldier->position().level() );
...
BOOLEAN UseBlade( TacticalActor *pSoldier , INT32 sTargetGridNo )
{
	...
	// Deduct points!
	sAPCost = CalcTotalAPsToAttack( pSoldier, sTargetGridNo, FALSE, (pSoldier->aiPlanning().aimTime() / 2) );
	...
	if ( pTargetSoldier )
	{
		...
		pTargetSoldier->targeting().intendedTarget() = TRUE;
		...
		{
			iHitChance = CalcChanceToStab( pSoldier, pTargetSoldier, pSoldier->aiPlanning().aimTime() );
		...
		if ( pSoldier->roster().team() == gbPlayerNum && pSoldier->identity().profile() != NO_PROFILE )
		{
			gMercProfiles[ pSoldier->identity().profile() ].records.usBladeAttacks++;
			if ( iDiceRoll < iHitChance )
			{
				gMercProfiles[ pSoldier->identity().profile() ].records.usShotsHit++;
			}
		}

		// anv: taunt on attack
		PossiblyStartEnemyTaunt( pSoldier, TAUNT_ATTACK_BLADE, pTargetSoldier->identity().id() );
		...
		if ( iDiceRoll < iHitChance )
		{
			...
			// anv: taunts on hit
			PossiblyStartEnemyTaunt( pSoldier, TAUNT_HIT_BLADE, pTargetSoldier->identity().id() );
			PossiblyStartEnemyTaunt( pTargetSoldier, TAUNT_GOT_HIT_BLADE, pSoldier->identity().id() );
			...
			AdjustImpactByHitLocation( iImpact, pSoldier->attackSelection().meleeLocation(), &iImpact, &iImpactForCrits );
			...
			if ( pTargetSoldier->identity().profile() != NO_PROFILE )
				gMercProfiles[ pTargetSoldier->identity().profile() ].records.usTimesWoundedStabbed++;

			// Send event for getting hit
			memset( &(SWeaponHit), 0, sizeof( SWeaponHit ) );
			SWeaponHit.usSoldierID			= pTargetSoldier->identity().id();
			SWeaponHit.uiUniqueId       = pTargetSoldier->identity().incarnation();
			...
			SWeaponHit.sZPos						= 20;
			SWeaponHit.sRange						= 1;
			SWeaponHit.ubAttackerID			= pSoldier->identity().id();
		...
		{
			// anv: taunts on miss
			PossiblyStartEnemyTaunt( pSoldier, TAUNT_MISS_BLADE, pTargetSoldier->identity().id() );
			if( pTargetSoldier->awareness().opponentKnowledge()[ pSoldier->identity().id() ] == SEEN_CURRENTLY )
				PossiblyStartEnemyTaunt( pTargetSoldier, TAUNT_GOT_MISSED_BLADE, pSoldier->identity().id() );

			// if it was another team shooting at someone under our control
			if ( pSoldier->roster().team() != pTargetSoldier->roster().team() )
			{
			...
			// FreeUpAttacker( (UINT8) pSoldier->identity().id() );
		}
		...
		if ( PTR_OURTEAM && pSoldier->targeting().targetId() != NOBODY)
		{
			...
			if ( pTargetSoldier->identity().bodyType() == COW ||
				pTargetSoldier->identity().bodyType() == CROW )
			{
				usExpGain /= 2;
			}
			else if ( pTargetSoldier->status().flags() &
					SOLDIER_VEHICLE ||
	...
	if ( pSoldier->perception().monsterSmell() > 0 && Random( 5 ) == 0 )
	{
		pSoldier->perception().monsterSmell()--;
	}
...
BOOLEAN UseHandToHand( TacticalActor *pSoldier, INT32 sTargetGridNo, BOOLEAN fStealing )
{
	...
	if ( pTargetSoldier )
	{
		...
		if (fStealing)
		{
			...
			if ( AllowedToStealFromTeamMate(pSoldier->identity().id(), pTargetSoldier->identity().id()) )
			{
				iHitChance = 100;
			}
			else if ( AM_A_ROBOT( pTargetSoldier ) || ENEMYROBOT( pTargetSoldier ) || ARMED_VEHICLE( pTargetSoldier ) || CREATURE_OR_BLOODCAT( pTargetSoldier ) || (SOLDIER_CLASS_MILITIA( pTargetSoldier->roster().soldierClass() ) && (gGameExternalOptions.ubMilitiaDropEquipment != 2)) ) // added militia here - SANDRO
			{
				iHitChance = 0;
			}
			...
			{
				iHitChance = CalcChanceToSteal( pSoldier, pTargetSoldier, pSoldier->aiPlanning().aimTime() );
			}
		}
		else
		{
			...
			{
				iHitChance = CalcChanceToPunch( pSoldier, pTargetSoldier, pSoldier->aiPlanning().aimTime() );
		...
		if (fStealing )
		{
			...
			else if ( pTargetSoldier->awareness().opponentKnowledge()[pSoldier->identity().id()] == SEEN_CURRENTLY )
				fNoticed = TRUE;
			else
			{
				UINT8 chance = 10;

				if ( pTargetSoldier->awareness().opponentKnowledge()[pSoldier->identity().id()] == SEEN_LAST_TURN )
					chance += 40;

				chance += pSoldier->aiBehavior().alertStatus() * 10;
			...
			if ( AllowedToStealFromTeamMate(pSoldier->identity().id(), pTargetSoldier->identity().id()) ||
				...
				pTargetSoldier->aiBehavior().alertStatus() < STATUS_RED ) ) ) ) )
			{
				fStealAttempt = TRUE;
				...
				usOldItem = pTargetSoldier->inventory()[HANDPOS].usItem;
				...
				if ( sNumStolenItems == 1)
				{
					...
					ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, Message[ STR_STOLE_SOMETHING ], pSoldier->GetName(), ShortItemNames[ pTargetSoldier->inventory()[ubIndexRet].usItem ] );
					if (pTargetSoldier->inventory()[ubIndexRet].MoveThisObjectTo(gTempObject, 1) == 0) {
					...
					// The item that the enemy holds in his hand before the stealing
					usNewItem = pTargetSoldier->inventory()[HANDPOS].usItem;
			...
			{
				if ( pTargetSoldier->inventory()[HANDPOS].exists() == true )
				{
					UINT16 usOldItemTarget = pTargetSoldier->inventory()[HANDPOS].usItem;

					fStealAttempt = TRUE; // added for check

					ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, Message[ STR_STOLE_SOMETHING ], pSoldier->GetName(), ShortItemNames[ pTargetSoldier->inventory()[HANDPOS].usItem ] );

					// if successful - place the item to our hands
					if (iDiceRoll < (iHitChance * 2 / 3) && gGameExternalOptions.fEnhancedCloseCombatSystem)
					{
						usOldItem = pSoldier->inventory()[HANDPOS].usItem;
						...
						{
							if( pSoldier->roster().team() != gbPlayerNum )  // for enemies, ignore rules and place it anywhere
							{
								bEmptySl = FindEmptySlotWithin( pSoldier, BIGPOCKSTART, NUM_INV_SLOTS );
								SwapObjs( pSoldier, HANDPOS, bEmptySl, TRUE );
							}
							else if (!PlaceInAnyPocket( pSoldier, &pSoldier->inventory()[HANDPOS], FALSE, 5))
							{
								if (!AutoPlaceObject( pSoldier, &pSoldier->inventory()[HANDPOS], FALSE, 5))
									AddItemToPool( pSoldier->position().gridNo(), &pSoldier->inventory()[HANDPOS], 1, pSoldier->position().level(), 0, -1 );
							}
						}
						// if twohanded item, place the secondhand item if any to temp space
						if (pSoldier->inventory()[SECONDHANDPOS].exists() == true && ItemIsTwoHanded(pTargetSoldier->inventory()[HANDPOS].usItem))
						{
							if( pSoldier->roster().team() != gbPlayerNum )  // for enemies, ignore rules and place it anywhere
							{
								bEmptySl = FindEmptySlotWithin( pSoldier, BIGPOCKSTART, NUM_INV_SLOTS );
							...
							{
								pSoldier->inventory()[SECONDHANDPOS].MoveThisObjectTo(oTempObjectSecondHand, 1);
							}
						}
						// now take the item
			...
			if (fFailure)
			{
				...
				if (pTargetSoldier->inventory()[HANDPOS].exists() == true)
				{
					ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE,
						...
						pSoldier->GetName(), ShortItemNames[ pTargetSoldier->inventory()[HANDPOS].usItem ] );
			...
			if ( iHitChance > 0 && pSoldier->roster().team() == gbPlayerNum && pTargetSoldier->roster().team() != gbPlayerNum && !(pTargetSoldier->status().flags() & SOLDIER_VEHICLE) && !AM_A_ROBOT( pTargetSoldier ) && !ARMED_VEHICLE( pTargetSoldier ) && !ENEMYROBOT( pTargetSoldier ) )
			{
				if (fFailure == FALSE)
				{
					...
					if ( fNoticed )
					{
						...
						PossiblyStartEnemyTaunt( pTargetSoldier, TAUNT_GOT_ROBBED, pSoldier->identity().id() );
					}
				}
				else
				{
			...
			if ( fNoticed )
			{
				...
				PossiblyStartEnemyTaunt( pSoldier, TAUNT_STEAL, pTargetSoldier->identity().id() );
			}
		}
		...
		else
		{
			...
			PossiblyStartEnemyTaunt( pSoldier, TAUNT_ATTACK_HTH, pTargetSoldier->identity().id() );

			// SANDRO - new mercs' records 
			if ( pSoldier->roster().team() == gbPlayerNum && pSoldier->identity().profile() != NO_PROFILE )
			{
				gMercProfiles[ pSoldier->identity().profile() ].records.usHtHAttacks++;
				if ( iDiceRoll < iHitChance )
				{
					gMercProfiles[ pSoldier->identity().profile() ].records.usShotsHit++;
				}
			...
			if ( pSoldier->roster().team() == gbPlayerNum && pTargetSoldier->roster().team() != gbPlayerNum )
			{
				// WDS 07/19/2008 - Random number use fix
				...
				if ( iDiceRoll < iHitChance )
				{
					...
					{ 
						...
						if (pSoldier->attackSelection().meleeLocation() == AIM_SHOT_HEAD)
						{
							ubExpGain += 2;
						}
						// Decreased EXP for leg strike
						else if (pSoldier->attackSelection().meleeLocation() == AIM_SHOT_LEGS)
						{
							ubExpGain -= 4;
						}
						
						// Double EXP for focused attack
						if ( (pSoldier->animationPlayback().state() == NINJA_SPINKICK || pSoldier->aiPlanning().aimTime() >= (gGameExternalOptions.fEnhancedCloseCombatSystem ? gSkillTraitValues.ubModifierForAPsAddedOnAimedPunches : 6)) &&
					...
					if ( pTargetSoldier->status().flags() & SOLDIER_VEHICLE || AM_A_ROBOT( pTargetSoldier ) || ARMED_VEHICLE( pTargetSoldier ) || ENEMYROBOT( pTargetSoldier ) )
					{
						ubExpGain = 0;
					}
					else if ( pTargetSoldier->identity().bodyType() == COW || pTargetSoldier->identity().bodyType() == CROW )
					{
						ubExpGain /= 2;
					}
			...
			else if ( pSoldier->roster().team() != gbPlayerNum && pTargetSoldier->roster().team() == gbPlayerNum )
			{
...
//Ja25 no meanwhiles
...
#endif
			{
				...
				if ( pTargetSoldier->identity().profile() != NO_PROFILE )
					gMercProfiles[ pTargetSoldier->identity().profile() ].records.usTimesWoundedPunched++;
				...
				PossiblyStartEnemyTaunt( pSoldier, TAUNT_HIT_HTH, pTargetSoldier->identity().id() );
				PossiblyStartEnemyTaunt( pTargetSoldier, TAUNT_GOT_HIT_HTH, pSoldier->identity().id() );
			}
			else
			{
				// anv: enemy taunts on miss
				PossiblyStartEnemyTaunt( pSoldier, TAUNT_MISS_HTH, pTargetSoldier->identity().id() );
				if( pTargetSoldier->awareness().opponentKnowledge()[ pSoldier->identity().id() ] == SEEN_CURRENTLY )
					PossiblyStartEnemyTaunt( pTargetSoldier, TAUNT_GOT_MISSED_HTH, pSoldier->identity().id() );
				...
				if (pTargetSoldier->actionPoints().current() > 0 &&
					...
					!(pTargetSoldier->featureFlags().secondaryFlags() & SOLDIER_BACK_ATTACK) &&
					(!pTargetSoldier->inventory()[HANDPOS].exists() || Item[pTargetSoldier->inventory()[HANDPOS].usItem].usItemClass & (IC_BLADE | IC_THROWING_KNIFE | IC_PUNCH) || !pTargetSoldier->inventory()[SECONDHANDPOS].exists()))
				{
					if (!(Item[pTargetSoldier->inventory()[HANDPOS].usItem].usItemClass & (IC_BLADE | IC_THROWING_KNIFE | IC_PUNCH)) &&
						!pTargetSoldier->inventory()[SECONDHANDPOS].exists())
					{
					...
					pTargetSoldier->aiPlanning().aimTime() = 0;
					if (pTargetSoldier->actionPoints().current() >= MinAPsToAttack(pTargetSoldier, pSoldier->position().gridNo(), TRUE, 1, 0) && Chance(EffectiveStrength(pTargetSoldier, FALSE) / 4))
						pTargetSoldier->aiPlanning().aimTime() = 1;
					...
					if (gAnimControl[pSoldier->animationPlayback().state()].ubEndHeight > ANIM_PRONE)
					{
						if (Chance((6 + EffectiveDexterity(pTargetSoldier, FALSE) / 10 + 5 * NUM_SKILL_TRAITS(pTargetSoldier, MARTIAL_ARTS_NT)) * 100 / (100 + pSoldier->vitals().breath())))
							pTargetSoldier->attackSelection().meleeLocation() = AIM_SHOT_HEAD;
						else if (Chance(pSoldier->vitals().breath() * EffectiveWisdom(pTargetSoldier) / (100 + EffectiveDexterity(pTargetSoldier, FALSE))))
							pTargetSoldier->attackSelection().meleeLocation() = AIM_SHOT_LEGS;
						else
							pTargetSoldier->attackSelection().meleeLocation() = AIM_SHOT_TORSO;
					}

					HandleItem(pTargetSoldier, pSoldier->position().gridNo(), pTargetSoldier->position().level(), pTargetSoldier->inventory()[HANDPOS].usItem, FALSE);
				}
			}
		}
	}
...
BOOLEAN UseThrown( TacticalActor *pSoldier, INT32 sTargetGridNo )
{
	...
	if ( pSoldier->roster().team() == gbPlayerNum && IsJa2TacticalCombatActive() )
	{
		...
		if ( pTargetSoldier && pTargetSoldier->roster().team() == pSoldier->roster().team() )
		{
			// ignore!
			pTargetSoldier = NULL;
		}
			...
			if ( ubTargetID != NOBODY )
			{
				pTargetSoldier =
					GetJa2SoldierRepository().resolve(
						ubTargetID.i);
				if ( pTargetSoldier->roster().team() != pSoldier->roster().team() )
				{
					usNumTargets++;
				}
			}
	...
	/////////////////////////////////////////////////////////////////////////////////////
	// SANDRO - new mercs' records
	if ( Item[ pSoldier->inventory()[HANDPOS].usItem ].usItemClass == IC_GRENADE || Item[ pSoldier->inventory()[HANDPOS].usItem ].usItemClass == IC_BOMB || Item[ pSoldier->inventory()[HANDPOS].usItem ].usItemClass == IC_THROWN)
	{
		gMercProfiles[ pSoldier->identity().profile() ].records.usGrenadesThrown++;
	}
	/////////////////////////////////////////////////////////////////////////////////////
	...
	if (ubVolume > 0)
	{
		...
		MakeNoise(pSoldier->identity().id(), pSoldier->position().gridNo(), pSoldier->position().level(), pSoldier->position().terrainType(), ubVolume, NOISE_GRENADE_IMPACT);
	}
...
BOOLEAN UseLauncher( TacticalActor *pSoldier, INT32 sTargetGridNo )
{
	...
	if ( !EnoughAmmo( pSoldier, TRUE, pSoldier->attackSelection().hand() ) )
	{
		return( FALSE );
	}
	...
	//if ( pSoldier->attackSelection().weapon() == pObj->usItem )
	//{
	...
	if ( pSoldier->attackSelection().weaponMode() & (WM_ATTACHED_GL|WM_ATTACHED_GL_BURST|WM_ATTACHED_GL_AUTO) )
	{
	...
	if ( WillExplosiveWeaponFail( pSoldier, pgunobj ) )
	{
		...
		INT16 sX, sY;
		ConvertGridNoToCenterCellXY(pSoldier->position().gridNo(), &sX, &sY);
		IgniteExplosion( pSoldier->identity().id(), sX, sY, 0, pSoldier->position().gridNo(), Launchable.usItem, pSoldier->position().level() );
	...
	if ( Item[ usItemNum ].usItemClass == IC_LAUNCHER )
	{
		...
		sAPCost = CalcTotalAPsToAttack( pSoldier, sTargetGridNo, FALSE, pSoldier->aiPlanning().shownAimTime() );
		// SANDRO: get BP cost for weapon manipulating
	...
	{
		gMercProfiles[ pSoldier->identity().profile() ].records.usMissilesLaunched++;
	}
	/////////////////////////////////////////////////////////////////////////////////////
...
void WeaponHit( SoldierID usSoldierID, UINT16 usWeaponIndex, INT16 sDamage, INT16 sBreathLoss, UINT16 usDirection, INT16 sXPos, INT16 sYPos, INT16 sZPos, INT16 sRange, SoldierID ubAttackerID, BOOLEAN fHit, UINT8 ubSpecial, UINT8 ubHitLocation )
{
	...
	// CALLAHAN END BUGFIX
	{
		...
		// marke test mag ammo type: pSoldier->inventory()[pSoldier->attackSelection().hand() ][0]->data.gun.ubGunAmmoType
                // 2cond 'or' added
		if (ItemIsRocketLauncher(usWeaponIndex) || AmmoTypes[ubAmmoType].explosionSize > 1 )
		{
			...
			{
				DebugMsg( TOPIC_JA2, DBG_LEVEL_3, String("WeaponHit: RPG7 item: %d, Ammo: %d",pSoldier->inventory()[HANDPOS].usItem , pSoldier->inventory()[HANDPOS][0]->data.gun.usGunAmmoItem ) );

				IgniteExplosion( ubAttackerID, sXPos, sYPos, 0, GETWORLDINDEXFROMWORLDCOORDS( sYPos, sXPos ), pSoldier->inventory()[pSoldier->attackSelection().hand() ][0]->data.gun.usGunAmmoItem, pTargetSoldier->position().level() );
				...
				OBJECTTYPE * pLaunchable = FindLaunchableAttachment( &(pSoldier->inventory()[pSoldier->attackSelection().hand() ]), pSoldier->inventory()[pSoldier->attackSelection().hand() ].usItem );
				if(pLaunchable)
				{
					pSoldier->inventory()[pSoldier->attackSelection().hand() ][0]->data.gun.usGunAmmoItem = pLaunchable->usItem;
				}
				else
				{
					pSoldier->inventory()[pSoldier->attackSelection().hand() ][0]->data.gun.usGunAmmoItem = NONE;
...
void StructureHit( INT32 iBullet, UINT16 usWeaponIndex, INT16 bWeaponStatus, SoldierID ubAttackerID, UINT16 sXPos, INT16 sYPos, INT16 sZPos, UINT16 usStructureID, INT32 iImpact, BOOLEAN fStopped )
{
	...
	if ( fStopped && ubAttackerID != NOBODY )
	{
		if ( pAttacker->targeting().hasEngagedOpponent() )
		{
			...
			// if it was another team shooting at someone under our control
			if ( pAttacker->roster().team() != opponent->roster().team() )
			{
				// if OPPONENT is under our control
				if ( opponent->roster().team() == gbPlayerNum )
				{
	...
	if (fStopped)
	{
		...
		if (ItemIsRocketLauncher(usWeaponIndex) || (pObj && AmmoTypes[ (*pObj)[0]->data.gun.ubGunAmmoType].explosionSize > 1 ))
		{
			...
			if ( pBullet->fFragment == false)
			{
				...
				{
					DebugMsg( TOPIC_JA2, DBG_LEVEL_3, String("StructureHit: RPG7 item: %d, Ammo: %d",pAttacker->inventory()[HANDPOS].usItem , pAttacker->inventory()[HANDPOS][0]->data.gun.usGunAmmoItem ) );
					IgniteExplosion( ubAttackerID, sX, sY, 0, sGridNo, pAttacker->inventory()[pAttacker->attackSelection().hand()][0]->data.gun.usGunAmmoItem, bLevel, usDirection );
					
					//This is just to make multishot launchers work in semi auto. It's not really a permanent solution because it still doesn't allow autofire, but it will do for now.
					OBJECTTYPE * pLaunchable = FindLaunchableAttachment( &(pAttacker->inventory()[pAttacker->attackSelection().hand() ]), pAttacker->inventory()[pAttacker->attackSelection().hand() ].usItem );
					if(pLaunchable)
					{
						pAttacker->inventory()[pAttacker->attackSelection().hand() ][0]->data.gun.usGunAmmoItem = pLaunchable->usItem;
					}
					else
					{
						pAttacker->inventory()[pAttacker->attackSelection().hand() ][0]->data.gun.usGunAmmoItem = NONE;
					}
				}
	...
	if (pBullet->fFragment == false)
	{
		switch(  Weapon[ usWeaponIndex ].ubWeaponClass )
		{
			...
			case MGCLASS:
				...
				if ( ubAttackerID != NOBODY &&
					pAttacker->roster().team() == gbPlayerNum )
				{
	...
	if ( fDoMissForGun )
	{
		...
		else
		{
			...
			if( pBullet->pFirer != NULL && pBullet->pFirer->targeting().hasEngagedOpponent() )
			{
				...
				SoldierID ShooterID = pBullet->pFirer->identity().id();

				if( Item[ pBullet->pFirer->attackSelection().weapon() ].usItemClass & IC_GUN )
				{
					...
					else if (opponent->awareness().opponentKnowledge()[ShooterID] ==
						SEEN_CURRENTLY)
					{
						PossiblyStartEnemyTaunt(
							opponent,
				...
				else if( Item[ pBullet->pFirer->attackSelection().weapon() ].usItemClass & IC_THROWING_KNIFE )
				{
					if( opponent->awareness().opponentKnowledge()[ShooterID] ==
						SEEN_CURRENTLY )
						PossiblyStartEnemyTaunt(
							opponent,
...
UINT32 CalcNewChanceToHitGun(TacticalActor *pSoldier, INT32 sGridNo, INT16 ubAimTime, UINT8 ubAimPos )
{
	...
	if (ubTargetID != NOBODY && (pSoldier->awareness().opponentKnowledge()[ubTargetID] == SEEN_CURRENTLY || gbPublicOpplist[pSoldier->roster().team()][ubTargetID] == SEEN_CURRENTLY))
	{
		iSightRange = SoldierToSoldierLineOfSightTest(
			...
			pSoldier->attackSelection().shotLocation(), false );
	}
	if (iSightRange == 0) 
	{	
	...
	{	
		...
		if (ubTargetID != NOBODY)
			iSightRange = SoldierToSoldierLineOfSightTest(
				pSoldier, targetSoldier, TRUE, NO_DISTANCE_LIMIT,
				pSoldier->attackSelection().shotLocation(), false, true );
		fCantSeeTarget = true;
	}
	...
	if (ubAimTime > 0)
	{
		...
		if ( !TacticalActorWeaponHandling::isValidAlternativeFireMode(*pSoldier,  ubAimTime, sGridNo ) )
		{
			...
			const FLOAT fHighestScope = GetHighestScopeMagnificationFactor( &(pSoldier->inventory()[pSoldier->attackSelection().hand()]) );
...
UINT32 CalcChanceToHitGun(TacticalActor *pSoldier, INT32 sGridNo, INT16 ubAimTime, UINT8 ubAimPos )
{
	...
	if(iSightRange > 0 && !TacticalActorWeaponHandling::isValidAlternativeFireMode(*pSoldier,  ubAimTime, sGridNo ) ){
		...
		if (iRange < GetMinRangeForAimBonus(pSoldier, &(pSoldier->inventory()[pSoldier->attackSelection().hand()])) && iScopeVisionRangeBonus > 50){	// iSightRange penalty for using a high power scope within min range due to poor focus
			iPenalty = 0;
			for(UINT8 loop = 0; loop < ((GetMinRangeForAimBonus(pSoldier, &(pSoldier->inventory()[pSoldier->attackSelection().hand()])) - iRange)/CELL_X_SIZE); loop++){
...
/*			case ANIM_STAND:
	...
	if (pTarget != NULL)
	{
		...
		iPenalty = __min(
			(UINT16)((float)pTarget->movementMetrics().tilesMoved() *
				(float)gGameExternalOptions.iMovementEffectOnAiming),
			gGameExternalOptions.usMaxCTHPenaltyForMovingTarget );
		///////////////////////////////////////////////////////////////////////////////////
		...
		if ( pTarget->awareness().opponentKnowledge()[pSoldier->identity().id()] == SEEN_CURRENTLY && !ARMED_VEHICLE( pTarget ) && !ENEMYROBOT( pTarget ) && !(pSoldier->identity().bodyType() != QUEENMONSTER) )
		{
			...
			if ( ARMED_VEHICLE( pTarget ) || ENEMYROBOT( pTarget ) || (pSoldier->identity().bodyType() != QUEENMONSTER) )
			{
	...
	attachmentList::iterator iter_end = (*&(pSoldier->inventory()[pSoldier->attackSelection().hand()]))[0]->attachments.end();
	for(attachmentList::iterator iter = (*&(pSoldier->inventory()[pSoldier->attackSelection().hand()]))[0]->attachments.begin(); iter != iter_end; ++iter)
	{
	...
	if ( Item[(&(pSoldier->inventory()[pSoldier->attackSelection().hand()]))->usItem].usItemClass == IC_GUN && !TacticalActorWeaponHandling::isValidAlternativeFireMode(*pSoldier,  ubAimTime, sGridNo ) )
	{
		...
		if ( ObjList[pSoldier->attackSelection().scopeMode()] != NULL )
		{
			iPenalty = (Item[ObjList[pSoldier->attackSelection().scopeMode()]->usItem].aimbonus * (iRange - Item[ObjList[pSoldier->attackSelection().scopeMode()]->usItem].minrangeforaimbonus)) / 1000;
			iPenalty = min(AIM_BONUS_PRONE, iPenalty);
			iChance -= iPenalty;
		}
	}
	...
	/////////////////////////////////////////////////////////////////////////////////////
	// AI Modifiers
	if ( !(pSoldier->status().flags() & SOLDIER_PC ) )	// if this is a computer AI controlled enemy
	{
	...
	if (pSoldier->roster().soldierClass() == SOLDIER_CLASS_GREEN_MILITIA && gGameExternalOptions.sGreenMilitiaCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sGreenMilitiaCtHBonusPercent) /100);
	else if (pSoldier->roster().soldierClass() == SOLDIER_CLASS_REG_MILITIA && gGameExternalOptions.sRegularMilitiaCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sRegularMilitiaCtHBonusPercent) /100);
	else if (pSoldier->roster().soldierClass() == SOLDIER_CLASS_ELITE_MILITIA && gGameExternalOptions.sVeteranMilitiaCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sVeteranMilitiaCtHBonusPercent) /100);
	// bonus for enemy
	else if ( (pSoldier->roster().soldierClass() == SOLDIER_CLASS_ADMINISTRATOR || pSoldier->roster().soldierClass() == SOLDIER_CLASS_BANDIT) && gGameExternalOptions.sEnemyAdminCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sEnemyAdminCtHBonusPercent) /100);
	else if (pSoldier->roster().soldierClass() == SOLDIER_CLASS_ARMY && gGameExternalOptions.sEnemyRegularCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sEnemyRegularCtHBonusPercent) /100);
	else if (pSoldier->roster().soldierClass() == SOLDIER_CLASS_ELITE && gGameExternalOptions.sEnemyEliteCtHBonusPercent != 0)
	...
	if (AM_A_ROBOT(pSoldier) && ItemProvidesRobotLaserBonus(pSoldier->inventory()[ROBOT_TARGETING_SLOT].usItem))
	{
		iChance += GetToHitBonus(&(pSoldier->inventory()[ROBOT_TARGETING_SLOT]), iRange, bLightLevel, FALSE);
	}
	else
	{
...
UINT32 AICalcChanceToHitGun(TacticalActor *pSoldier, INT32 sGridNo, INT16 ubAimTime, UINT8 ubAimPos, INT8 bTargetLevel, UINT16 usAnimState)//dnl ch59 180813
{
	...
	if(Item[pSoldier->attackSelection().weapon()].usItemClass & IC_THROWING_KNIFE)//dnl ch70 160913
	{
	...
	if(UsingNewCTHSystem() && !(Item[pSoldier->attackSelection().weapon()].usItemClass & IC_THROWING_KNIFE))//dnl ch70 160913
	{
		////////////////////////////////////////////////////////////////////////////////////
		...
		FLOAT dMagFactor = CalcMagFactor( pSoldier, &(pSoldier->inventory()[pSoldier->attackSelection().hand()]), d2DDistance, sGridNo, (UINT8)ubAimTime );
		...
		INT16 sLaserRange = GetBestLaserRange( &(pSoldier->inventory()[pSoldier->attackSelection().hand()]) );
		if (sLaserRange > 0 
...
INT32 TotalArmourProtection( TacticalActor * pTarget, UINT8 ubHitLocation, INT32 iImpact, UINT8 ubAmmoType, BOOLEAN fConsiderFlak )
{
	...
	if (pTarget->status().flags() & SOLDIER_VEHICLE)
	{
	...
	else
	{
		...
		pVestPack = &(pTarget->inventory()[VESTPOCKPOS]);
			...
			for (attachmentList::iterator iter = (*pVestPack)[0]->attachments.begin(); iter != (*pVestPack)[0]->attachments.end(); ++iter) {
				if (Item[iter->usItem].usItemClass == IC_ARMOUR && (*iter)[0]->data.objectStatus > 0 )
				{
					...
					if ( (*iter)[0]->data.objectStatus < USABLE )
					{
						...
						if ( pTarget->roster().team() == gbPlayerNum )
						{
							ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, gzLateLocalizedString[61], pTarget->identity().name() );
						}
					}
					break;
				}
		...
		{
			pArmour = &(pTarget->inventory()[ iSlot ]);
...
INT32 BulletImpact( TacticalActor *pFirer, BULLET *pBullet, TacticalActor * pTarget, UINT8 ubHitLocation, INT32 iOrigImpact, INT16 sHitBy, UINT8 * pubSpecial )
{
	...
	{
		usAttackingWeapon = pFirer->inventory()[pFirer->attackSelection().hand()][0]->data.gun.ubGunAmmoType;
		sOrigGridNo = pFirer->position().gridNo();

		ammoitem = pFirer->inventory()[pFirer->attackSelection().hand()][0]->data.gun.usGunAmmoItem;
	}
	else
	{
	...
	else if ( pTarget->status().flags() & SOLDIER_VEHICLE || pTarget->status().flags() & SOLDIER_ROBOT )
	{
	...
	{
		FLOAT robotDamageReduction = Item[pTarget->inventory()[ROBOT_CHASSIS_SLOT].usItem].fRobotDamageReductionModifier;
		if (robotDamageReduction > 0.0f && robotDamageReduction < 1.0f)
		{
			...
			damagefactor *= Item[pTarget->inventory()[ROBOT_CHASSIS_SLOT].usItem].fRobotDamageReductionModifier;
			OBJECTTYPE* pRobotPlate = &(pTarget->inventory()[ROBOT_CHASSIS_SLOT]);
...
//		iOrigImpact = AMMO_DAMAGE_ADJUSTMENT_HE( iOrigImpact );
	...
	if ( !ARMED_VEHICLE( pTarget ) && !(pTarget->status().flags() & SOLDIER_VEHICLE) && AmmoTypes[ubAmmoType].ammoflag & AMMO_CRYO )
	{
	...
	if ( iImpact > 0 && !ARMED_VEHICLE( pTarget ) && !ENEMYROBOT( pTarget ) )
	{
		...
		if ( gGameOptions.fNewTraitSystem && pFirer ) // new traits bonuses - SANDRO
		{
			...
			{
				iImpact += (INT32)(iImpact * (gSkillTraitValues.ubTHBladesDamageBonus + (gSkillTraitValues.ubTHBladesDamageBonusPerClick * pFirer->aiPlanning().aimTime()))/100);
			}
	
			// Sniper trait adds bonus damage per aim click
			if (HAS_SKILL_TRAIT( pFirer, SNIPER_NT ) && (pFirer->aiPlanning().aimTime() >= gSkillTraitValues.ubSNDamageBonusFromNumClicks) && !fFragment)
			{	
				iImpact += (INT32)(iImpact * (pFirer->aiPlanning().aimTime() - gSkillTraitValues.ubSNDamageBonusFromNumClicks + 1) * gSkillTraitValues.ubSNDamageBonusPerClick * NUM_SKILL_TRAITS( pFirer, SNIPER_NT ))/100; // +5% per trait
			}
		}
		...
		switch( ubHitLocation )
		{
			case AIM_SHOT_HEAD:
				...
				//if ( PythSpacesAway( pFirer->sGridNo, pTarget->sGridNo ) <= MAX_DISTANCE_FOR_MESSY_DEATH || (PythSpacesAway( pFirer->sGridNo, pTarget->sGridNo ) <= MAX_BARRETT_DISTANCE_FOR_MESSY_DEATH && pFirer->attackSelection().weapon()  == BARRETT ))
	...
	{
		if ( AmmoTypes[ubAmmoType].knife && pFirer && pFirer->awareness().opponentKnowledge()[ pTarget->identity().id() ] == SEEN_CURRENTLY && !fFragment )
		{
			// is this a stealth attack?
			if ( pTarget->awareness().opponentKnowledge()[ pFirer->identity().id() ] == NOT_HEARD_OR_SEEN && !CREATURE_OR_BLOODCAT( pTarget )
				&& (ubHitLocation == AIM_SHOT_HEAD || ubHitLocation == AIM_SHOT_TORSO ) && (pTarget->aiBehavior().alertStatus() < STATUS_RED || !gGameExternalOptions.fEnhancedCloseCombatSystem) ) // Added check if enemy is alerted - SANDRO
			{
		...
		if (iImpactForCrits > 0 && iImpactForCrits < pTarget->vitals().health() && !(pTarget->status().flags() & SOLDIER_MONSTER) && !(pTarget->status().flags() & SOLDIER_VEHICLE) && (!(gTacticalStatus.uiFlags & GODMODE) || pTarget->roster().team() != OUR_TEAM))
		{
			...
			{
				uiCritChance = (iImpactForCrits / 2) + (pFirer->aiPlanning().aimTime() * 5);
			}
			...
			if ((PreRandom( uiCritChance ) + 1) > (UINT8)(gGameOptions.fNewTraitSystem ? gSkillTraitValues.ubDamageNeededToLoseStats : CRITICAL_HIT_THRESHOLD))
			{
				...
				if ( fMaliciousHit && Chance( max( 15, uiCritChance )) && ( ubHitLocation == AIM_SHOT_TORSO || ubHitLocation == AIM_SHOT_LEGS ) && 
					( sHitBy >= 20 ) && ( pTarget->identity().bodyType() <= STOCKYMALE ) && ( gAnimControl[ pTarget->animationPlayback().state() ].ubHeight != ANIM_PRONE ) )
				{
				...
				else
				{
					switch( ubHitLocation )
					{
						case AIM_SHOT_HEAD:
							...
							if ( bStatLoss > 0 )
							{
								...
								{
									gMercProfiles[ pTarget->identity().profile() ].bWisdom = pTarget->statistics().wisdom();
								}
						...
						case AIM_SHOT_TORSO:
							...
							if ( PreRandom( 5 ) == 0 )
							{
								...
								if ( bStatLoss > 0 )
								{
									...
									if (pTarget->identity().profile() != NO_PROFILE)
									{
										gMercProfiles[ pTarget->identity().profile() ].bLifeMax = pTarget->vitals().maximumHealth();
									}
							...
							else
							{
								...
								if ( Chance(50) )
								{
									...
									if ( bStatLoss > 0 )
									{
										...
										if (pTarget->identity().profile() != NO_PROFILE)
										{
											gMercProfiles[ pTarget->identity().profile() ].bDexterity = pTarget->statistics().dexterity();
										}
								...
								else
								{
									...
									if ( bStatLoss > 0 )
									{
										...
										if (pTarget->identity().profile() != NO_PROFILE)
										{
											gMercProfiles[ pTarget->identity().profile() ].bStrength = pTarget->statistics().strength();
										}
						...
						case AIM_SHOT_LEGS:
							...
							if ( bStatLoss > 0 )
							{
								...
								if (pTarget->identity().profile() != NO_PROFILE)
								{
									gMercProfiles[ pTarget->identity().profile() ].bAgility = pTarget->statistics().agility();
								}
					...
					if ( bStatLoss > 0 && pTarget->identity().profile() != NO_PROFILE )
						gMercProfiles[ pTarget->identity().profile() ].records.usTimesStatDamaged++;
				}
			}
...
INT32 HTHImpact( TacticalActor * pSoldier, TacticalActor * pTarget, INT32 iHitBy, BOOLEAN fBladeAttack )
{
	...
	if (!fBladeAttack)
	{
		if ( gGameOptions.fNewTraitSystem )
		{
			if (!pSoldier->attackSelection().weapon() || ItemIsBrassKnuckles(pSoldier->inventory()[HANDPOS].usItem))
			{
				if ( gGameExternalOptions.fEnhancedCloseCombatSystem )
				{
				...
				else
				{
					...
					if ( HAS_SKILL_TRAIT( pSoldier, MARTIAL_ARTS_NT ) )
					{
						...
						if ( !autoresolve &&
							(pSoldier->animationPlayback().state() == NINJA_SPINKICK || (pSoldier->aiPlanning().aimTime() >= (gGameExternalOptions.fEnhancedCloseCombatSystem ? gSkillTraitValues.ubModifierForAPsAddedOnAimedPunches : 6)) ))
						{
							//iImpact = (INT32)((iImpact * (100 + gSkillTraitValues.usMAAimedPunchDamageBonus * NUM_SKILL_TRAITS( pSoldier, MARTIAL_ARTS_NT ) ) / 100) + 0.5); // +75% damage per trait
			...
			else
			{
				...
				if (HAS_SKILL_TRAIT( pSoldier, MELEE_NT ))
				{
					...
					if ( !autoresolve && pSoldier->aiPlanning().aimTime() >= (gGameExternalOptions.fEnhancedCloseCombatSystem ? gSkillTraitValues.ubModifierForAPsAddedOnAimedBladedAttackes : 6) )
					{
		...
		if ( !autoresolve && gGameExternalOptions.fEnhancedCloseCombatSystem)
		{
			...
			if (pTarget->awareness().opponentKnowledge()[ pSoldier->identity().id() ] == NOT_HEARD_OR_SEEN && !CREATURE_OR_BLOODCAT( pTarget ) )
			{
	...
	if ( !autoresolve && pObj && HasItemFlag(pObj->usItem, GAROTTE) )
	{
		...
		if ( pTarget->aiBehavior().alertStatus() == STATUS_YELLOW )
			resistchance += 20;
		else if ( pTarget->aiBehavior().alertStatus() >= STATUS_RED )
			resistchance += 50;
...
void ShotMiss( SoldierID ubAttackerID, INT32 iBullet )
{
	...
	switch( Weapon[pAttacker->attackSelection().weapon()].ubWeaponClass )
	{
		case HANDGUNCLASS:
		case RIFLECLASS:
		case SHOTGUNCLASS:
		...
		case MGCLASS:
			...
			// Guy has missed, play random sound
			if ( pAttacker->roster().team() == gbPlayerNum )
			{
				if ( Random(40) == 0 )
				{
					pAttacker->DoMercBattleSound(
	...
	if ( fDoMissForGun )
	{
		...
		if ( !DoSpecialEffectAmmoMiss(
				ubAttackerID, pAttacker->attackSelection().weapon(),
	...
	if ( pAttacker != NULL && pAttacker->targeting().hasEngagedOpponent() )
	{
		...
		{
			if(opponent->awareness().opponentKnowledge()[ pAttacker->identity().id() ] ==
				SEEN_CURRENTLY )
				PossiblyStartEnemyTaunt(
					opponent, TAUNT_GOT_MISSED_GUNFIRE,
					pAttacker->identity().id());
			PossiblyStartEnemyTaunt( pAttacker, TAUNT_MISS_GUNFIRE, OpponentID );
		}
		else if( Item[ pAttacker->attackSelection().weapon() ].usItemClass & IC_THROWING_KNIFE )
		{
			if( opponent->awareness().opponentKnowledge()[ pAttacker->identity().id() ] ==
				SEEN_CURRENTLY )
				PossiblyStartEnemyTaunt(
...
UINT32 CalcChanceHTH( TacticalActor * pAttacker,TacticalActor *pDefender, INT16 ubAimTime, UINT8 ubMode )
{
	...
	usInHand = pAttacker->attackSelection().weapon();
	...
	if (! (pAttacker->status().flags() & SOLDIER_PC) )   // if attacker is a computer AI controlled enemy
	{
		iAttRating += gbDiff[ DIFF_ENEMY_TO_HIT_MOD ][ SoldierDifficultyLevel( pAttacker ) ];
	}

	// if attacker is being affected by gas
	if ( pAttacker->status().flags() & SOLDIER_GASSED )
	...
	else
	{
		...
		if ( gGameOptions.fNewTraitSystem )
		{
			...
			if (!pAttacker->attackSelection().weapon() && HAS_SKILL_TRAIT( pAttacker, MARTIAL_ARTS_NT ))
			{
			...
			else if (ItemIsBrassKnuckles(pAttacker->inventory()[HANDPOS].usItem) && HAS_SKILL_TRAIT( pAttacker, MARTIAL_ARTS_NT ))
			{
	...
	if ( gGameOptions.fNewTraitSystem && pAttacker->identity().profile() != NO_PROFILE )
	{
		// Sociable - better performance in groups
		if ( DoesMercHavePersonality( pAttacker, CHAR_TRAIT_SOCIABLE ) )
		{	
			INT8 bNumMercs = CheckMercsNearForCharTraits( pAttacker->identity().profile(), CHAR_TRAIT_SOCIABLE );
			if ( bNumMercs > 2 )
				iAttRating += 5;
			else if ( bNumMercs > 0 )
				iAttRating += 2;
		...
		{	
			INT8 bNumMercs = CheckMercsNearForCharTraits( pAttacker->identity().profile(), CHAR_TRAIT_LONER );
			if ( bNumMercs == 0 )
				iAttRating += 5;
			else if ( bNumMercs <= 1 )
				iAttRating += 2;
		...
		{	
			INT8 bNumMercs = CheckMercsNearForCharTraits( pAttacker->identity().profile(), CHAR_TRAIT_SHOWOFF );
			if ( bNumMercs > 1 )
				iAttRating += 5;
			else if ( bNumMercs > 0 )
				iAttRating += 2;
	...
	// if attacker is being affected by gas
	if ( pDefender->status().flags() & SOLDIER_GASSED )
	...
	if ( usInHand == CREATURE_QUEEN_TENTACLES && (pDefender->identity().bodyType() == LARVAE_MONSTER || pDefender->identity().bodyType() == INFANT_MONSTER) )   // && binds tighter than || -> was protecting ALL infants regardless of weapon
	{
	...
	if (ubMode == HTH_MODE_STAB)
	{
		if ( gGameOptions.fNewTraitSystem )
		{
			...
			if (HAS_SKILL_TRAIT( pDefender, MELEE_NT ))
			{
				...
				if (Item[pDefender->inventory()[HANDPOS].usItem].usItemClass == IC_BLADE)
				{
	...
	{	// punch/hand-to-hand/martial arts attack/steal
		if ( gGameOptions.fNewTraitSystem )
		{
			...
			if ( !pAttacker->attackSelection().weapon() || ItemIsBrassKnuckles(pAttacker->inventory()[HANDPOS].usItem) )
			{
				if (HAS_SKILL_TRAIT( pDefender, MARTIAL_ARTS_NT ))
				{
					...
					if ( !pDefender->inventory()[HANDPOS].exists() )
					{
						iDefRating += ( gSkillTraitValues.ubMAOnTopCTDHtHBareHanded * NUM_SKILL_TRAITS( pDefender, MARTIAL_ARTS_NT ) );
					}
					else if (ItemIsBrassKnuckles(pDefender->inventory()[HANDPOS].usItem))
					{
		...
		else
		{
			...
			if (HAS_SKILL_TRAIT( pDefender, KNIFING_OT ) && Item[pDefender->inventory()[HANDPOS].usItem].usItemClass == IC_BLADE && ubMode != HTH_MODE_STEAL)
			{
	...
	// Added character traits influence
	if ( gGameOptions.fNewTraitSystem && pDefender->identity().profile() != NO_PROFILE )
	{
		// Sociable - better performance in groups
		if ( DoesMercHavePersonality( pDefender, CHAR_TRAIT_SOCIABLE ) )
		{	
			INT8 bNumMercs = CheckMercsNearForCharTraits( pDefender->identity().profile(), CHAR_TRAIT_SOCIABLE );
			if ( bNumMercs > 2 )
				iDefRating += 5;
			else if ( bNumMercs > 0 )
				iDefRating += 2;
		...
		{	
			INT8 bNumMercs = CheckMercsNearForCharTraits( pDefender->identity().profile(), CHAR_TRAIT_LONER );
			if ( bNumMercs == 0 )
				iDefRating += 5;
			else if ( bNumMercs <= 1 )
				iDefRating += 2;
		...
		{	
			INT8 bNumMercs = CheckMercsNearForCharTraits( pDefender->identity().profile(), CHAR_TRAIT_SHOWOFF );
			if ( bNumMercs > 1 )
				iDefRating += 5;
			else if ( bNumMercs > 0 )
				iDefRating += 2;
	...
	else
	{
		...
		if (pAttacker->status().flags() & SOLDIER_BOXER)
		...
		if (gGameExternalOptions.fEnhancedCloseCombatSystem)
		{
			if( gGameOptions.fNewTraitSystem  && HAS_SKILL_TRAIT( pAttacker, MARTIAL_ARTS_NT ) && 
				((pAttacker->animationPlayback().state() == NINJA_SPINKICK) || (pAttacker->aiPlanning().aimTime() >= (gGameExternalOptions.fEnhancedCloseCombatSystem ? gSkillTraitValues.ubModifierForAPsAddedOnAimedPunches : 6))) )
			{
			...
			if ( pAttacker->attackSelection().meleeLocation() == AIM_SHOT_HEAD )
			{
				// Harder to hit head (but higher damage there)
				iChance -= 20;
			}
			else if( pAttacker->attackSelection().meleeLocation() == AIM_SHOT_LEGS )
			{
	...
	////////////////////////////////////////////////////////////////////////////////////
	// SANDRO - Bonus CtH for Militia
	if (pAttacker->roster().soldierClass() == SOLDIER_CLASS_GREEN_MILITIA && gGameExternalOptions.sGreenMilitiaCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sGreenMilitiaCtHBonusPercent) /100);
	else if (pAttacker->roster().soldierClass() == SOLDIER_CLASS_REG_MILITIA && gGameExternalOptions.sRegularMilitiaCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sRegularMilitiaCtHBonusPercent) /100);
	else if (pAttacker->roster().soldierClass() == SOLDIER_CLASS_ELITE_MILITIA && gGameExternalOptions.sVeteranMilitiaCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sVeteranMilitiaCtHBonusPercent) /100);
	// bonus for enemy
	else if ( ( pAttacker->roster().soldierClass() == SOLDIER_CLASS_ADMINISTRATOR || pAttacker->roster().soldierClass() == SOLDIER_CLASS_BANDIT ) && gGameExternalOptions.sEnemyAdminCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sEnemyAdminCtHBonusPercent) /100);
	else if (pAttacker->roster().soldierClass() == SOLDIER_CLASS_ARMY && gGameExternalOptions.sEnemyRegularCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sEnemyRegularCtHBonusPercent) /100);
	else if (pAttacker->roster().soldierClass() == SOLDIER_CLASS_ELITE && gGameExternalOptions.sEnemyEliteCtHBonusPercent != 0)
		iChance += ((iChance * gGameExternalOptions.sEnemyEliteCtHBonusPercent) /100);
	//////////////////////////////////////////////////////////////////////////////////////

	// sevenfm: bonus for boxers for attacking from the back
	if (ubMode == HTH_MODE_PUNCH &&
		(pAttacker->status().flags() & SOLDIER_BOXER) &&
		(pDefender->status().flags() & SOLDIER_BOXER) &&
		iChance < 100 &&
...
void ReloadWeapon( TacticalActor *pSoldier, UINT8 ubHandPos )
{
	...
	if ( pSoldier->inventory()[ ubHandPos ].exists() == true )
	{
		pSoldier->inventory()[ ubHandPos ][0]->data.gun.ubGunShotsLeft = GetMagSize(&pSoldier->inventory()[ ubHandPos ]);
		// Dirty Bars
		if ( IsWeaponAttached(&pSoldier->inventory()[ ubHandPos ], IC_GUN) )
		{
			(*FindAttachedWeapon(&pSoldier->inventory()[ ubHandPos ], IC_GUN))[0]->data.gun.ubGunShotsLeft = GetMagSize(FindAttachedWeapon(&pSoldier->inventory()[ ubHandPos ], IC_GUN));
		}
...
BOOLEAN IsGunWeaponModeCapable( OBJECTTYPE* pObject, WeaponMode bWpnMode, TacticalActor *pSoldier )
{
	...
	{
		case WM_NORMAL:
		//return (pSoldier->inventory()[ ubHandPos ].exists() == true && Item[ pSoldier->inventory()[ ubHandPos ].usItem ].usItemClass & IC_WEAPON && !Weapon[ pSoldier->inventory()[ ubHandPos ].usItem ].NoSemiAuto );  // Check for being a weapon....
		...
		case WM_AUTOFIRE:
		//return ((IsGunAutofireCapable(pSoldier, ubHandPos) || Weapon[ pSoldier->inventory()[ ubHandPos ].usItem ].NoSemiAuto )&& !Item[pSoldier->inventory()[ubHandPos].usItem].grenadelauncher );
		return ((IsGunAutofireCapable(pObject) || Weapon[ pObject->usItem ].NoSemiAuto )&& !ItemIsGrenadeLauncher(pObject->usItem));

		case WM_ATTACHED_GL:
//		return (FindAttachment( &(pSoldier->inventory()[ubHandPos]), UNDER_GLAUNCHER ) != 0 && FindLaunchableAttachment( &(pSoldier->inventory()[ubHandPos]), UNDER_GLAUNCHER ) != 0 );

		//return (!Item[pSoldier->inventory()[ubHandPos].usItem].grenadelauncher &&  IsGrenadeLauncherAttached( &(pSoldier->inventory()[ubHandPos]) ) && FindLaunchableAttachment( &(pSoldier->inventory()[ubHandPos]), GetAttachedGrenadeLauncher( &(pSoldier->inventory()[ubHandPos]) )) != 0 );
		...
		case WM_ATTACHED_GL_BURST:
			//return (!Item[pSoldier->inventory()[ubHandPos].usItem].grenadelauncher && IsGrenadeLauncherAttached( &(pSoldier->inventory()[ubHandPos]) ) && Weapon[GetAttachedGrenadeLauncher(&pSoldier->inventory()[ubHandPos])].ubShotsPerBurst > 0 && FindLaunchableAttachment( &(pSoldier->inventory()[ubHandPos]), GetAttachedGrenadeLauncher( &(pSoldier->inventory()[ubHandPos]))) != 0 );
		...
		case WM_ATTACHED_GL_AUTO:
			return FALSE;
//			return (!Item[pSoldier->inventory()[ubHandPos].usItem].grenadelauncher && Weapon[GetAttachedGrenadeLauncher(&pSoldier->inventory()[ubHandPos])].bAutofireShotsPerFiveAP > 0 && FindLaunchableAttachment( &(pSoldier->inventory()[ubHandPos]), GetAttachedGrenadeLauncher( &(pSoldier->inventory()[ubHandPos]) ) != ITEM_NOT_FOUND );
...
BOOLEAN IsGunAutofireCapable( OBJECTTYPE* pObject )
{
	...
	if ( pObject->exists() == true )
	{
		...
		//if ( Item[ pSoldier->inventory()[ ubHandPos ].usItem ].usItemClass & IC_WEAPON )
		if ( Item[ pObject->usItem ].usItemClass & IC_WEAPON )
		{
			//if ( GetAutofireShotsPerFiveAPs(&pSoldier->inventory()[ ubHandPos ]) > 0 )
...
BOOLEAN IsGunBurstCapable(OBJECTTYPE* pObject, BOOLEAN fNotify, TacticalActor* pSoldier )
{
	...
	if ( pObject->exists() == true )
	{
		...
		if ( Item[ pObject->usItem ].usItemClass & IC_WEAPON )
		{
			//if ( GetShotsPerBurst(&pSoldier->inventory()[ ubHandPos ]) > 0 )
			if ( GetShotsPerBurst(pObject) > 0 )
			{
				...
				if ( gGameOptions.fNewTraitSystem && gSkillTraitValues.fCanFanTheHammer && Weapon[pObject->usItem].fBurstOnlyByFanTheHammer )
				{
					...
					if ( pSoldier && 
						...
						!pSoldier->inventory()[SECONDHANDPOS].exists() )
					{
						fCapable = TRUE;
					}
				}
...
UINT32 CalcThrownChanceToHit(TacticalActor *pSoldier, INT32 sGridNo, INT16 ubAimTime, UINT8 ubAimPos )
{
	...
	if ( pSoldier->attackSelection().weaponMode() == WM_ATTACHED_GL || pSoldier->attackSelection().weaponMode() == WM_ATTACHED_GL_BURST || pSoldier->attackSelection().weaponMode() == WM_ATTACHED_GL_AUTO )
	{
		usHandItem = GetAttachedGrenadeLauncher (&pSoldier->inventory()[HANDPOS]);// UNDER_GLAUNCHER;
	}
	else
	{
		usHandItem = pSoldier->inventory()[HANDPOS].usItem;
	}
...
*/
	...
	if ( Item[ usHandItem ].usItemClass != IC_LAUNCHER && pSoldier->attackSelection().weaponMode() != WM_ATTACHED_GL && pSoldier->attackSelection().weaponMode() != WM_ATTACHED_GL_BURST && pSoldier->attackSelection().weaponMode() != WM_ATTACHED_GL_AUTO )
	{
		...
		if ( gGameOptions.fNewTraitSystem )
		{
			...
			if ( Item[ usHandItem ].usItemClass == IC_THROWING_KNIFE )
			{
				...
				if ( HAS_SKILL_TRAIT( pSoldier, THROWING_NT ) )
					iChance += (gSkillTraitValues.ubTHBladesCtHBonus + (gSkillTraitValues.ubTHBladesCtHBonusPerClick * pSoldier->aiPlanning().aimTime()) );
			}
			else
			{
	...
	else
	{
		...
		if (pSoldier->roster().team() == OUR_TEAM ||
			pSoldier->roster().team() == ENEMY_TEAM && gGameExternalOptions.fAssignTraitsToEnemy ||
			pSoldier->roster().team() == MILITIA_TEAM && gGameExternalOptions.fAssignTraitsToMilitia)
		{
			// SANDRO - old/new traits
			if (gGameOptions.fNewTraitSystem)
			{
	...
	if ( UsingNewCTHSystem() == false && pSoldier->fireControl().burstCounter() )
	{
		int iPenalty = ( Weapon[usHandItem].ubBurstPenalty
			- GetBurstToHitBonus(&pSoldier->inventory()[HANDPOS], gAnimControl[ pSoldier->animationPlayback().state() ].ubEndHeight == ANIM_PRONE) )
			* (pSoldier->fireControl().burstCounter() - 1);
...
*/
	...
	/*if ( GetDayVisionRangeBonus (pSoldier) >0) // pSoldier->inventory()[HEAD1POS].usItem == SUNGOGGLES || pSoldier->inventory()[HEAD2POS].usItem == SUNGOGGLES)
	{
	...
	{
		iMaxRange = CalcMaxTossRange( pSoldier, usHandItem , TRUE , &pSoldier->inventory()[HANDPOS]) * CELL_X_SIZE;
	...
	if ((iChance > 0) && (Item[ usHandItem ].usItemClass == IC_LAUNCHER ))
		...
		iChance = (iChance * WEAPON_STATUS_MOD(pSoldier->inventory()[HANDPOS][0]->data.objectStatus)) / 100;
...
void ChangeWeaponMode( TacticalActor * pSoldier )
{
	...
	else
	{
		...
		pSoldier->fireControl().selectBarrelMode( max( 1, pSoldier->fireControl().barrelMode() ) );
		UINT8 nextbarrelmode = GetNextBarrelMode( pSoldier->inventory()[HANDPOS].usItem, pSoldier->fireControl().barrelMode() );
		...
		else
		{
			...
			do
			{
				pSoldier->attackSelection().weaponMode()++;

				if ( pSoldier->attackSelection().weaponMode() == NUM_WEAPON_MODES )
				{
					if ( Weapon[pSoldier->inventory()[HANDPOS].usItem].NoSemiAuto )
						pSoldier->attackSelection().weaponMode() = WM_AUTOFIRE;
					else
					...
					{
						OBJECTTYPE* pRifleGrenadeDeviceObj = FindAttachment_GrenadeLauncher( &( pSoldier->inventory()[HANDPOS] ) );

						if ( pRifleGrenadeDeviceObj && FindLaunchableAttachment( &( pSoldier->inventory()[HANDPOS] ), pRifleGrenadeDeviceObj->usItem ) )
						{
			...
			// Changed by ADB, rev 1513
			//while(IsGunWeaponModeCapable( pSoldier, HANDPOS, pSoldier->attackSelection().weaponMode() ) == FALSE && pSoldier->attackSelection().weaponMode() != WM_NORMAL);
			while ( IsGunWeaponModeCapable( &pSoldier->inventory()[HANDPOS], static_cast<WeaponMode>( pSoldier->attackSelection().weaponMode() ), pSoldier ) == FALSE && pSoldier->attackSelection().weaponMode() != WM_NORMAL );
		}
	}
	
	if (pSoldier->attackSelection().weaponMode() == WM_AUTOFIRE || pSoldier->attackSelection().weaponMode() == WM_ATTACHED_GL_AUTO || pSoldier->attackSelection().weaponMode() == WM_ATTACHED_UB_AUTO)
	{
...
void ChangeScopeMode( TacticalActor * pSoldier, INT32 iTrgGridNo )
{
	...
	INT8 bOldAimTime = pSoldier->aiPlanning().shownAimTime();
	...
	else
	{
		...
		do
		{
			...
			if(pSoldier->attackSelection().scopeMode() == NUM_SCOPE_MODES)
			{
				// SANDRO - this is where we offer firing from alternative weapon hold
				pSoldier->attackSelection().scopeMode() = USE_ALT_WEAPON_HOLD; // USE_BEST_SCOPE;
			}
		}
		while( ObjList[pSoldier->attackSelection().scopeMode()] == NULL && pSoldier->attackSelection().scopeMode() != USE_ALT_WEAPON_HOLD); //USE_BEST_SCOPE);
		
		// If this mode is not allowed, or we are not standing, or holding something weird, skip it
		if ( pSoldier->attackSelection().scopeMode() == USE_ALT_WEAPON_HOLD && ( gGameExternalOptions.ubAllowAlternativeWeaponHolding != 3 || gAnimControl[ pSoldier->animationPlayback().state() ].ubEndHeight != ANIM_STAND || AM_A_ROBOT(pSoldier) || ItemIsRocketLauncher(pSoldier->inventory()[HANDPOS].usItem) || ItemIsSingleShotRocketLauncher(pSoldier->inventory()[HANDPOS].usItem)) )
			pSoldier->attackSelection().scopeMode() = USE_BEST_SCOPE;
	}
	...
	if ( iTrgGridNo != NOWHERE )
	{
		...
		{
			pSoldier->aiPlanning().shownAimTime() = AllowedAimingLevels(pSoldier, iTrgGridNo);
		}
	}
	...
	if ( Weapon[pSoldier->inventory()[HANDPOS].usItem].fBurstOnlyByFanTheHammer && pSoldier->attackSelection().weaponMode() == WM_BURST || pSoldier->attackSelection().weaponMode() == WM_ATTACHED_GL_BURST || pSoldier->attackSelection().weaponMode() == WM_ATTACHED_UB_BURST )
	{
		if ( !IsGunWeaponModeCapable( &pSoldier->inventory()[HANDPOS], static_cast<WeaponMode>( pSoldier->attackSelection().weaponMode() ), pSoldier ) )
		{
...
void DishoutQueenSwipeDamage( TacticalActor *pQueenSoldier )
{
	...
	for ( cnt = 0; cnt < Ja2ActiveTacticalActorSlotCount(); cnt++ )
	{
		...
		if (pSoldier != NULL )
		{
			if ( pSoldier->identity().id() != pQueenSoldier->identity().id() )
			{
...
void CalcMagFactorSimple( TacticalActor *pSoldier, FLOAT d2DDistance, INT16 bAimTime, INT32 iGridNo )
{
	...
	if (AM_A_ROBOT(pSoldier) && ItemProvidesRobotLaserBonus(pSoldier->inventory()[ROBOT_TARGETING_SLOT].usItem))
	{
		gCTHDisplay.iBestLaserRange = max(gCTHDisplay.iBestLaserRange, GetBestLaserRange(&pSoldier->inventory()[ROBOT_TARGETING_SLOT]));
	}
...
FLOAT CalcNewChanceToHitBaseSpecialBonus(TacticalActor *pSoldier)
{
	...
	// GAME DIFFICULTY
	if ( !(pSoldier->status().flags() & SOLDIER_PC ) && (pSoldier->roster().side() != gbPlayerNum) )
	{
		fBaseModifier += zDiffSetting[gGameOptions.ubDifficultyLevel].fCTHSettingsBaseDifficultyEnemy;
	}
	else if ( pSoldier->roster().team() == MILITIA_TEAM )
	{
...
BOOLEAN ArtilleryStrike( UINT16 usItem, SoldierID ubOwnerID, UINT32 usStartingGridNo, UINT32 usTargetMapPos )
{
	...
	/*if (is_networked && is_client)
	{
		if (pSoldier->roster().team() == 0 || (pSoldier->roster().team() == 1 && is_server))
		{
			send_grenade( &shellobj , dLifeSpan,dX, dY, dZ, dForceX, dForceY, dForceZ, sTargetGridNo, pSoldier->identity().id(), ubActionCode, uiActionData, iID , false);
		}
	}*/
