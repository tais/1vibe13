...
static TacticalActor* FindSoldierByProfileID2(UINT8 ubProfileID, BOOLEAN fPlayerMercsOnly)
{
	...
	for (; soldier <= gTacticalStatus.Team[CIV_TEAM].bLastID; ++soldier)
	{
		...
		if (candidate && candidate->roster().active() && candidate->roster().inSector())
		{
			return candidate;
		}
	}
...
static int l_usStrategicInsertionDataProfileID(lua_State* L)
{
	...
	if (lua_gettop(L))
	{
		...
		TacticalActor* pSoldier = FindSoldierByProfileID(ID, FALSE);
		if (pSoldier)
		{
	...
	if (lua_gettop(L) >= 2)
	{
		...
		else
		{
			...
			for (; cnt2 <= gTacticalStatus.Team[CIV_TEAM].bLastID; ++cnt2)
			{
				...
				if (pSoldier->roster().active() && pSoldier->roster().inSector() && pSoldier->identity().profile() == NO_PROFILE)
				{
					pSoldier->aiPlanning().actionInProgress() = ExecuteAction(pSoldier);
				}
			}
...
static int l_SetOffBombsByFrequency(lua_State* L)
{
	...
	if (lua_gettop(L) >= 2)
	{
		...
		if (ACTION >= 1 || ACTION <= 4)
		{
			...
			//if (pSoldier)
			//{
			SetOffBombsByFrequency(ID /*pSoldier->identity().id()*/, FIRST_MAP_PLACED_FREQUENCY + ACTION);
			//}
		}
	}
	return 0;
...
static int l_TeleportSoldier(lua_State* L)
{
	if (lua_gettop(L) >= 2)
	{
		...
		if (ubID < NUM_PROFILES && !TileIsOutOfBounds(sGridNo))
		{
			TacticalActor* pSoldier = FindSoldierByProfileID(ubID, TRUE);
			if (pSoldier)
			{
...
static int l_EVENT_InitNewSoldierAnim(lua_State* L)
{
	if (lua_gettop(L) >= 3)
	{
		...
		{
			TacticalActor* pSoldier = FindSoldierByProfileID(ubTargetNPC, TRUE);
...
static int l_SetEnterCombatMode(lua_State* L)
{
	if (lua_gettop(L) >= 2)
	{
		...
		for (SoldierID ubLoop = gTacticalStatus.Team[CIV_TEAM].bFirstID; ubLoop <= gTacticalStatus.Team[CIV_TEAM].bLastID; ++ubLoop)
		{
			...
			if (pGoon->roster().civilianGroup() == group && pGoon->roster().active() && pGoon->roster().inSector() && pGoon->vitals().health() >= OKLIFE && pGoon->awareness().opponentKnowledge()[ubID] == SEEN_CURRENTLY)
			{
...
static int l_SetSoldierSide(lua_State* L)
{
	if (lua_gettop(L) >= 2)
	{
		...
		TacticalActor* pSoldier = FindSoldierByProfileID(UID, FALSE);
		if (pSoldier)
		{
			pSoldier->roster().side() = bNewSide;
		}
...
static int l_CheckSoldierCivilianGroup(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (pSoldier)
		{
			bGroup = pSoldier->roster().civilianGroup();
...
static int l_CheckSoldierubProfile(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (pSoldier)
		{
			bGroup = pSoldier->identity().profile();
...
static int l_CheckSoldierInSector(lua_State* L)
{
	...
	{
		UINT8 ubID = lua_tointeger(L, 1);
		TacticalActor* pSoldier = FindSoldierByProfileID(ubID, FALSE);
		BOOLEAN Bool = FALSE;
...
static int l_ChangeMercPtrsTeam(lua_State* L)
{
	if (lua_gettop(L) >= 2)
	{
		...
		if (pSoldier && pSoldier->roster().inSector() && pSoldier->roster().active())
		{
			pSoldier->roster().side() = Side;
		}
	}
...
static int l_FindItem2Soldier(lua_State* L)
{
	...
	if (lua_gettop(L) >= 3)
	{
		...
		TacticalActor* pSoldier = FindSoldierByProfileID(ubTargetNPC, fPlayerMercsOnly);
		if (pSoldier)
		{
	...
	if (lua_gettop(L))
	{
		...
		if (pSoldier)
		{
			...
			if (pSoldier->roster().team() == OUR_TEAM)
				...
				team = 4;
			else if (pSoldier->roster().team() == PLAYER_PLAN)
				team = 5;
...
static int l_ubID(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (pSoldier)
			lua_pushinteger(L, pSoldier->identity().id());
		else
			lua_pushinteger(L, -1);
	}
...
static int l_CheckMercPtrsActive(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (ubID != NOBODY && GetJa2SoldierRepository().resolve(ubID)->roster().active())
			Bool = TRUE;
		else
			Bool = FALSE;
...
static int l_CheckMercPtsrInSector(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (ubID != NOBODY && GetJa2SoldierRepository().resolve(ubID)->roster().inSector())
			Bool = TRUE;
		else
			Bool = FALSE;
...
static int l_GetMercPtsrProfileID(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (ubID != NOBODY)
		{
			UINT8 profil = GetJa2SoldierRepository().resolve(ubID)->identity().profile();
...
static int l_SoldierGiveItem(lua_State* L)
{
	if (lua_gettop(L) >= 3)
	{
		...
		TacticalActor* pSoldier = FindSoldierByProfileID(ubID1, FALSE);
		TacticalActor* pSoldier2 = FindSoldierByProfileID(ubID2, FALSE);
...
static int l_WhichBuddy(lua_State* L)
{
	if (lua_gettop(L) >= 2)
	{
		...
		if (firstSoldier && firstSoldier->roster().inSector() &&
			secondSoldier && secondSoldier->roster().inSector())
			bBuddyIndex = WhichBuddy(
				firstSoldier->identity().profile(), secondSoldier->identity().profile());
...
static int l_AnimMercPtsrsAbsoluteFinalDestination(lua_State* L)
{
	if (lua_gettop(L) >= 2)
	{
		...
		if (pSoldier && pSoldier->roster().inSector() && GridNo != NOWHERE)
...
static int l_SetSoldierBodyType(lua_State* L)
{
	if (lua_gettop(L) >= 2)
	{
		...
		if (pSoldier && pSoldier->roster().inSector() && pSoldier->roster().active())
			pSoldier->identity().bodyType() = Anim;
	}
	return 0;
}
...
static int l_GetSoldierBodyType(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (pSoldier && pSoldier->roster().inSector() && pSoldier->roster().active())
			Anim = pSoldier->identity().bodyType();
...
static int l_CheckMercPtrsInCivilianGroup(lua_State* L)
{
	if (lua_gettop(L))
	{
		...
		if (ubID != NOBODY)
		{
			UINT8 group = GetJa2SoldierRepository().resolve(ubID)->roster().civilianGroup();
			lua_pushinteger(L, group);
		}
	}
	return 1;
...
static int l_FindSoldierByProfileID(lua_State* L)
{
	...
	if (lua_gettop(L))
	{
		...
		for (ubLoop = 0; ubLoop < ubLoopLimit; ubLoop++)
		{
			...
			if (pSoldier->roster().active() && pSoldier->identity().profile() == ubTargetNPC)
			{
				foundProfileID = pSoldier->identity().profile();
				break;
			}
		}
...
static int l_ubWhatKindOfMercAmI(lua_State* L)
{
	...
	{
		UINT8 ubTargetNPC = lua_tointeger(L, 1);
		TacticalActor* pSoldier = FindSoldierByProfileID(ubTargetNPC, TRUE);//, fPlayerMercsOnly);
...
static int l_CreateArmedCivilain(lua_State* L)
{
	if (lua_gettop(L) >= 4)
	{
		...
		{
			TacticalActor* pSoldier = TacticalCreateArmedCivilian(usSoldierClass);
			...
			if (pSoldier)
			{
				...
				if (usCivilianGroup)
				{
					...
					if (fHostile)
						gTacticalStatus.fCivGroupHostile[pSoldier->roster().civilianGroup()] = CIV_GROUP_WILL_BECOME_HOSTILE;
				}
			}
		}
	}
...
static int l_CreateCivilian(lua_State* L)
{
	if (lua_gettop(L) >= 12)
	{
		...
		if (gGameExternalOptions.bExtraCivilians)
		{
			TacticalActor* pSoldier = TacticalCreateCivilian(sGridNo, usCivilianGroup, usTraderID, sBodyType, aVest, aPants, aHair, aSkin, item1, item2, item3, item4);
