...
void ImproveDirective(const RebelCommandDirectives directive)
{
	...
	CHAR16 text[200];
	sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_IMPROVE_DIRECTIVE_PROMPT], cost);
...
void SetupAdminActionBox(const UINT8 actionIndex, const UINT16 descriptionText, const UINT16 buttonText)
{
	...
	if (actionIndex < 5 || adminActionChangeState == CAAS_INIT)
	{
		...
		else // show button
		{
			...
			sgp_swprintf(text, std::size(text),szRebelCommandText[level == 0 ? RCT_ADMIN_ACTION_ESTABLISH : RCT_ADMIN_ACTION_IMPROVE], szRebelCommandAdminActionsText[buttonText]);
		...
		y += 22;
		sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_ADMIN_ACTION_TIER], rebelCommandSaveInfo.regions[iCurrentRegionId].GetLevel(actionIndex));
		...
		btnId = CreateTextButton(szRebelCommandText[RCT_CONFIRM], FONT10ARIAL, FONT_MCOLOR_LTYELLOW, FONT_BLACK, BUTTON_USE_DEFAULT, x+70, y, 70, 18, BUTTON_TOGGLE, MSYS_PRIORITY_HIGH, DEFAULT_MOVE_CALLBACK, [](GUI_BUTTON* btn, INT32 reason)
			{
				ButtonHelper(btn, reason, [btn]()
				{
					...
					CHAR16 text[200];
					sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_CONFIRM_CHANGE_ADMIN_ACTION_PROMPT], ADMIN_ACTION_CHANGE_COST);
					DoLapTopMessageBox(MSG_BOX_LAPTOP_DEFAULT, text, LAPTOP_SCREEN, MSG_BOX_FLAG_YESNO, [](UINT8 exitValue) {
						if (exitValue == MSG_BOX_RETURN_YES)
						{
	...
	MSYS_DefineRegion(&adminActionHelpTextRegion[actionIndex], x, helpTextY, x + 140, helpTextY + 68, MSYS_PRIORITY_HIGH,
		CURSOR_LAPTOP_SCREEN, [](MOUSE_REGION* pRegion, INT32 iReason)
		{
			...
			case RCAA_SAFEHOUSES:
				// 1 tile range
				sgp_swprintf(text, std::size(text),szRebelCommandHelpText[RCHT_AA_TOWN_PLUS_ONE]);
				break;
			...
			case RCAA_HARRIERS:
				// 1-2 tile range
				sgp_swprintf(text, std::size(text),szRebelCommandHelpText[RCHT_AA_TOWN_PLUS_TWO]);
				break;

			case RCAA_SCOUTS:
				// 2-3 tile range
				sgp_swprintf(text, std::size(text),szRebelCommandHelpText[RCHT_AA_TOWN_PLUS_THREE]);
				break;

			default:
				// apply in cities only
				sgp_swprintf(text, std::size(text),szRebelCommandHelpText[RCHT_AA_TOWN_ONLY]);
				break;
			}
	...
	MSYS_DefineRegion(&suppliesIncomeHelpTextRegion, usPosX, usPosY, usPosX + 100, usPosY + 35, MSYS_PRIORITY_HIGH,
		CURSOR_LAPTOP_SCREEN, [](MOUSE_REGION* pRegion, INT32 iReason) { 
			if (iReason == MSYS_CALLBACK_REASON_MOVE)
			{
				...
				sgp_swprintf(text, std::size(text),szRebelCommandHelpText[RCHT_SUPPLIES_INCOME], base);
				...
				for (int a = FIRST_TOWN+1; a < NUM_TOWNS; ++a)
				{
					...
					if (upkeepCount > 0)
					{
						...
						sgp_swprintf(text, std::size(text),L"%s\n-%d (%s)", text, totalUpkeep, pTownNames[a]);
					}
				}
...
BOOLEAN SetupMissionAgentBox(UINT16 x, UINT16 y, INT8 index)
{
	...
	if (agentIndex[index] < static_cast<INT8>(mercs.size()))
	{
		...
		case RCAM_DISRUPT_ASD:
		{
			...
			floatModifier *= 100.f;
			sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_ASD_INCOME_REDUCTION], floatModifier, L"%s", locSkillText[floatModifierSkill]);
			...
			if (gGameOptions.fNewTraitSystem)
			{
				...
				case MissionHelpers::DISRUPT_ASD_STEAL_FUEL:
				{
					...
					sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_STEAL_FUEL], pTownNames[townId], locSkillText[TECHNICIAN_NT]);
					agentBonusText.push_back(text);
				}
				break;
				...
				{
					sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_DESTROY_RESERVES], locSkillText[DEMOLITIONS_NT]);
					agentBonusText.push_back(text);
				}
				break;
				}
		...
		case RCAM_REDUCE_UNALERTED_ENEMY_VISION:
		{
			...
			floatModifier *= 100.f;
			CHAR16 text[100];
			sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_UNALERTED_VISION_PENALTY], floatModifier, L"%s", locSkillText[floatModifierSkill]);
			agentBonusText.push_back(text);
		}
		break;
		...
		case RCAM_SABOTAGE_INFANTRY_EQUIPMENT:
		{
			...
			CHAR16 text[100];
			sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_INFANTRY_GEAR_QUALITY], intModifier, locSkillText[intModifierSkill]);
			agentBonusText.push_back(text);
		}
		break;
		...
		case RCAM_SABOTAGE_MECHANICAL_UNITS:
		{
			...
			CHAR16 text[100];
			sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_MECHANICAL_STAT_LOSS], intModifier, locSkillText[intModifierSkill]);
			agentBonusText.push_back(text);
		}
		break;
		...
		case RCAM_SOLDIER_BOUNTIES_KINGPIN:
		{
			...
			if (floatModifier > 1.f)
			{
				floatModifier *= 100;
				floatModifier -= 100;
				sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_PAYOUT], floatModifier, L"%s", locSkillText[floatModifierSkill]);
				agentBonusText.push_back(text);
			}

			if (intModifier > 0)
			{
				sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_PAYOUT_LIMIT_INCREASE], intModifier, locSkillText[intModifierSkill]);
				agentBonusText.push_back(text);
			}

			if (extraBits == MissionHelpers::SOLDIER_BOUNTIES_KINGPIN_OFFICER_PAYOUTS)
			{
				sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_OFFICER_PAYOUT], locSkillText[floatModifierSkill]);
				agentBonusText.push_back(text);
			}
			else if (extraBits == MissionHelpers::SOLDIER_BOUNTIES_KINGPIN_VEHICLE_PAYOUTS)
			{
				sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_VEHICLE_PAYOUT], locSkillText[floatModifierSkill]);
				agentBonusText.push_back(text);
			}
		}
		break;
		...
		case RCAM_TRAIN_MILITIA_ANYWHERE:
		{
			...
			CHAR16 text[100];
			sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_BONUS_MAX_TRAINERS], intModifier, locSkillText[intModifierSkill]);
			agentBonusText.push_back(text);
		}
		break;
...
void PrepareMission(INT8 index)
{
	...
	sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_POPUP_PART1], szRebelCommandAgentMissionsText[missionTitle], cost);
	...
	if (agentIndex[index] == mercs.size())
	{
		...
		sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_POPUP_PART2_GENERIC], text);
	}
	else
	{
		...
		if (merc->bSex == MALE)
			sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_POPUP_PART2_MALE], text, merc->zNickname, gRebelCommandSettings.iMissionPrepTime);
		else
			sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MISSION_POPUP_PART2_FEMALE], text, merc->zNickname, gRebelCommandSettings.iMissionPrepTime);
	}
...
void RaidMines(INT32 &playerIncome, INT32 &enemyIncome)
{
	...
	{
		CHAR16 text[200];
		sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MINE_RAID_SUCCESSFUL], stolenIncome);
...
void DailyUpdate()
{
	...
	if (intelGain > 0)
	{
		sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_DEAD_DROP_INCOME], intelGain);
		ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"%s", text);
	...
	if (supplyGain > 0)
	{
		sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_SMUGGLER_INCOME], supplyGain);
		ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"%s", text );
...
void UpgradeMilitiaStats()
{
	...
	CHAR16 text[200];
	sgp_swprintf(text, std::size(text),szRebelCommandText[RCT_MILITIA_UPGRADE_STATS_PROMPT], cost);
	DoLapTopMessageBox(MSG_BOX_LAPTOP_DEFAULT, text, LAPTOP_SCREEN, MSG_BOX_FLAG_YESNO, [](UINT8 exitValue) {
		if (exitValue == MSG_BOX_RETURN_YES)
		{
			const INT32 cost = gRebelCommandSettings.iMilitiaUpgradeCosts[rebelCommandSaveInfo.iMilitiaStatsLevel++];
