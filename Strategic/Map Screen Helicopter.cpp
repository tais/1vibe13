...
static void PrepareHeliInfoDocument(void*)
{
...
BOOLEAN ReadInHeliInfo(STR fileName)
{
	...
	const LegacyXmlCallbacks callbacks{
		&pData, helisiteStartElementHandle, helisiteEndElementHandle,
		helisiteCharacterDataHandle, PrepareHeliInfoDocument};
...
UINT8 MoveAllInHelicopterToFootMovementGroup( INT8 bNewSquad )
{
	...
	for( iCounter = 0; iCounter < gNewVehicle[ pVehicleList[ iHelicopterVehicleId ].ubVehicleType ].iNewSeatingCapacities; ++iCounter )
	{
		...
		if( pSoldier != NULL )
		{
			...
			pSoldier->featureFlags().primaryFlags() |= (SOLDIER_AIRDROP_TURN|SOLDIER_AIRDROP);
		}
	}
