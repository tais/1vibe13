...
UINT8 gszTerrain[NUM_TRAVTERRAIN_TYPES][15] =
{
	...
	"TROPICS", "FARMLAND", "PLAINS_ROAD", "SPARSE_ROAD", "FARMLAND_ROAD",
	"TROPICS_ROAD", "DENSE_ROAD", "COASTAL", "HILLS_ROAD", "COASTAL_ROAD",
	"SAND_ROAD", "SWAMP_ROAD", "SPARSE_SAM", "SAND_SAM", "TROPICS_SAM",
	"MEDUNA_SAM", "CAMBRIA_HOSP", "DRASSEN_AIR", "MEDUNA_AIR", "SAM_SITE",
	"REFUEL_SITE", "REBEL_HIDEOUT", "TIXA_DUNGEON", "CREATURE_LAIR",
	"ORTA_BASEMENT", "TUNNEL", "SHELTER", "ABANDONED_MINE",
	"FINAL_COMPLEX", "GUARD_POST", "CRASH_SITE", "POWER_PLANT", "MOUNTAINS",
	"UNKNOWN"
};
...
BOOLEAN AddPlayerToGroup( UINT8 ubGroupID, TacticalActor *pSoldier )
{
	...
	if( !pGroup->pPlayerList )
	{
		...
		pGroup->ubPrevX = SECTORX( pSoldier->deployment().previousSectorId() );
		pGroup->ubPrevY = SECTORY( pSoldier->deployment().previousSectorId() );
		pGroup->ubSectorX = (UINT8)pSoldier->deployment().sectorX();
		pGroup->ubSectorY = (UINT8)pSoldier->deployment().sectorY();
		pGroup->ubSectorZ = (UINT8)pSoldier->deployment().sectorZ();
...
BOOLEAN RemovePlayerFromPGroup( GROUP *pGroup, TacticalActor *pSoldier )
{
	...
	{ //possibly the only node
		...
		pGroup->ubGroupSize--;
		pSoldier->deployment().previousSectorId() = (UINT8)SECTOR( pGroup->ubPrevX, pGroup->ubPrevY );
	...
	if ( pGroup->usGroupTeam == OUR_TEAM )
	{
		...
		else	// vehicle player group
		{
			...
			if( iVehId != iHelicopterVehicleId )
			{
				...
				pSoldier->deployment().insertionDirection() = ubInsertionDirection;
...
void HandleOtherGroupsArrivingSimultaneously( UINT8 ubSectorX, UINT8 ubSectorY, UINT8 ubSectorZ, GROUP* pArrivingGroup )
{
	...
	if ( pArrivingGroup && !IsGroupTheHelicopterGroup( pArrivingGroup ) )
	{
		...
		for ( uiCnt = 0; uiCnt <= gTacticalStatus.Team[gbPlayerNum].bLastID; ++uiCnt )
		{
			...
			if ( pSoldier && pSoldier->roster().active() && pSoldier->vitals().health() >= OKLIFE && SPY_LOCATION( pSoldier->assignment().current() ) )
			{
				if ( ( pSoldier->deployment().sectorX() == ubSectorX ) && ( pSoldier->deployment().sectorY() == ubSectorY ) && ( pSoldier->deployment().sectorZ() - 10 == ubSectorZ ) )
				{
...
void AddFuelToVehicle( TacticalActor *pSoldier, TacticalActor *pVehicle )
{
	...
	if( !ItemIsGascan(pItem->usItem))
	{
		#ifdef JA2BETAVERSION
			CHAR16 str[ 100 ];
			swprintf( str, L"%s is supposed to have gas can in hand. ATE:0", pSoldier->identity().name() );
			DoScreenIndependantMessageBox( str, MSG_BOX_FLAG_OK, NULL );
		#endif
		return;
	}
		...
		if ( fBloodCatAmbushPrevented )
		{
			for( SoldierID i = gTacticalStatus.Team[ OUR_TEAM ].bFirstID; i <= gTacticalStatus.Team[ OUR_TEAM ].bLastID; ++i )
			{
				...
				if( pSoldier->roster().active() && pSoldier->vitals().health() && !(pSoldier->status().flags() & SOLDIER_VEHICLE) )
				{
					...
					{
						if( HAS_SKILL_TRAIT( pSoldier, SCOUTING_NT ) && pSoldier->identity().profile() != NO_PROFILE )
						{
