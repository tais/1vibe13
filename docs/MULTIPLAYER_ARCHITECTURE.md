...
remain distinct even at the same sector coordinates. Paths, drive prefixes,
unterminated strings, nonzero padding, and missing loaded-sector names are
rejected before publication or replica commit; an unloaded sector requires an
empty key. Native capture takes this name from the successful LoadWorld filename
without reconstructing it from coordinates. Both sides of a delta sector change
carry the key, and the replica compares the previous key before committing.
This foundation transports identity; asset loading and static rendering are a
