...
- Runtime language identity now has one typed, data-free catalog for all eight
  ...
  The second slice retires all 58 catalog campaign/build guards. Thirteen typed
  keys classify 98 conditioned entries and 196 exact literal alternatives
  across the six affected tables; twelve keys occur in every language, while
  the historically guarded file-sender key occurs only in Dutch and French.
...
- `CommandJournal` records a bounded, best-effort history of submitted command
  ...
  command. The first-party stat-change notification now uses a private typed
  queue payload with exact `bool`/`INT16`/`UINT8` fields. The queue derives its
  existing `0x00400000` priority bit from that payload, while the raw numeric
  lane remains intact for Lua and legacy composites. The other 31 effects and
  raw stat-change compatibility remain legacy. In particular, `ENABLE_AI`
  ...
  It therefore remains named non-positional dialogue command debt.
...
- `Strategic/LuaInitNPCs.cpp` now compiles its exact 40 formerly guarded UB
  ...
  host, and has no remaining `JA2UB` guard. `IniFunction` retains all
  40 + 2 + 28 public-name/function mappings at their original three positions,
  but reads one fresh value-only `CampaignLuaGlobalPolicy` immediately after
  `InitMercFace`, caches one registration decision, and reuses it across all
  three direct gates. Arulco exports none of those names; UB exports all 70 in
  the established order. Editor state is orthogonal and unknown campaign
  values fail closed. The 512 common registrations, exact 34 live callers,
  two `bQuests` gates, and overwrite order of four duplicate common names are
  unchanged.
  A dependency-free fake registrar pins the 70 unique names, 42 callbacks,
  28 compatibility alias pairs, late single read, and invocation freeze and
  refresh; source ratchets pin the complete 582-entry sequence. Fourteen
