...
BOOLEAN DrawTacticalStripDepthSprite(
	UINT32 destinationSurface,
	PIXEL* destination,
	UINT32 destinationPitchBytes,
	UINT16* depthBuffer,
	...
	SGPRect* clipping)
{
	if (depthProfileFrame >= 0 &&
		BltVideoObjectStripDepthToSurface(
			destinationSurface, source, frame,
			static_cast<UINT16>(depthProfileFrame),
			destinationX, destinationY, depth,
			sameDepthPasses, obscured, clipping))
		return TRUE;
...
BOOLEAN DrawTacticalStripPaletteShadowDepthSprite(
	UINT32 destinationSurface,
	PIXEL* destination,
	UINT32 destinationPitchBytes,
	UINT16* depthBuffer,
	...
	SGPRect* clipping)
{
	if (depthProfileFrame >= 0 &&
		BltVideoObjectStripPaletteShadowDepthToSurface(
			...
			static_cast<UINT16>(depthProfileFrame),
			destinationX, destinationY, depth, palette,
			ignoreShadows, obscured, clipping))
		return TRUE;
	if (depthProfileFrame < 0) return FALSE;
	...
	if (obscured)
	{
		return alphaSource ?
			Blt8BPPDataTo16BPPBufferTransZTransShadowIncObscureClipAlpha(
				...
				depthBuffer, depth, source, alphaSource,
				destinationX, destinationY, frame, clipping,
				depthProfileFrame, palette, ignoreShadows) :
								...
								else
								{
									if (fMultiTransShadowZBlitter)
									{
										if (fZBlitter)
										{
											if (fObscuredBlitter)
											{
												DrawTacticalStripPaletteShadowDepthSprite(
													FRAME_BUFFER,
													(PIXEL*)pDestBuf,
													uiDestPitchBYTES,
													gpZBuffer, sZLevel,
								...
								if (fRenderTile && !(uiFlags&TILES_DIRTY))
								{
									if (fMerc)
									{
										if (pSoldier != NULL && pSoldier->identity().id() >= MAX_NUM_SOLDIERS)
										{
											SetFont(TINYFONT1);
											SetFontDestBuffer(guiSAVEBUFFER, 0, gsVIEWPORT_WINDOW_START_Y, SCREEN_WIDTH, gsVIEWPORT_WINDOW_END_Y, FALSE);
											VarFindFontCenterCoordinates(sXPos, sYPos, 1, 1, TINYFONT1, &sX, &sY, L"%d", pSoldier->uiPresentation().plannedActionPointCost());
											mprintf_buffer(pDestBuf, uiDestPitchBYTES, TINYFONT1, sX, sY, L"%d", pSoldier->uiPresentation().plannedActionPointCost());
											SetFontDestBuffer(FRAME_BUFFER, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, FALSE);
										}
									}
								}
